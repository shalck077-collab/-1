let currentSortOrder = "desc";
const { jsPDF } = window.jspdf;
// لصق الخط العربي هنا مباشرة (يجب استبدال 'YOUR_ARABIC_FONT_BASE64' بقيمة base64 لخط Amiri-Regular.ttf)
const arabicFontBase64 = '';
if (arabicFontBase64 !== 'YOUR_ARABIC_FONT_BASE64') {
    const callAddFont = function () {
        this.addFileToVFS('Amiri-Regular.ttf', arabicFontBase64);
        this.addFont('Amiri-Regular.ttf', 'Amiri', 'normal');
        this.setFont('Amiri', 'normal');
    };
    jsPDF.API.events.push(['addFonts', callAddFont]);
} else {
    console.warn('Arabic font base64 not provided. PDF generation may not display Arabic text correctly.');
}
// نظام إدارة المجلدات والملفات
class FileSystemManager {
    constructor() {
        this.mainFolder = 'العمل';
        this.subFolders = {
            buildings: 'العماير',
            workers: 'العمال', 
            partnership: 'الشراكة',
            backups: 'النسخ الاحتياطية'
        };
        this.initializeFileSystem();
    }

    initializeFileSystem() {
        // إنشاء المجلد الرئيسي والمجلدات الفرعية
        this.createFolderStructure();
    }

    createFolderStructure() {
        // محاكاة إنشاء المجلدات في النظام
        const folders = JSON.parse(localStorage.getItem('folderStructure') || '{}');
        
        if (!folders[this.mainFolder]) {
            folders[this.mainFolder] = {
                created: new Date().toISOString(),
                subFolders: {}
            };
            
            Object.values(this.subFolders).forEach(folderName => {
                folders[this.mainFolder].subFolders[folderName] = {
                    created: new Date().toISOString(),
                    files: []
                };
            });
            
            localStorage.setItem('folderStructure', JSON.stringify(folders));
        }
    }

    getFilePath(type, filename) {
        const folderName = this.subFolders[type] || this.subFolders.backups;
        return `${this.mainFolder}/${folderName}/${filename}`;
    }

    saveFileReference(type, filename, metadata = {}) {
        const folders = JSON.parse(localStorage.getItem('folderStructure') || '{}');
        const folderName = this.subFolders[type];
        
        if (folders[this.mainFolder] && folders[this.mainFolder].subFolders[folderName]) {
            folders[this.mainFolder].subFolders[folderName].files.push({
                name: filename,
                created: new Date().toISOString(),
                metadata: metadata
            });
            
            localStorage.setItem('folderStructure', JSON.stringify(folders));
        }
    }
}

// إنشاء مثيل من مدير الملفات
const fileManager = new FileSystemManager();

// وظائف الجداول الديناميكية
class DynamicTable {
    constructor(tableId) {
        this.table = document.getElementById(tableId);
        this.sortDirection = {};
        this.initializeSorting();
    }

    initializeSorting() {
        const headers = this.table.querySelectorAll('.sortable-header');
        headers.forEach((header, index) => {
            header.addEventListener('click', () => this.sortTable(index, header));
        });
    }

    sortTable(columnIndex, headerElement) {
        const tbody = this.table.querySelector('tbody');
        const rows = Array.from(tbody.rows);
        
        const direction = this.sortDirection[columnIndex] || 'asc';
        const newDirection = direction === 'asc' ? 'desc' : 'asc';
        
        rows.sort((a, b) => {
            const aValue = a.cells[columnIndex].textContent.trim();
            const bValue = b.cells[columnIndex].textContent.trim();
            
            let comparison = 0;
            
            // التحقق من كون القيمة رقماً
            const aNum = parseNumber(aValue);
            const bNum = parseNumber(bValue);
            
            if (!isNaN(aNum) && !isNaN(bNum)) {
                comparison = aNum - bNum;
            } else {
                // ترتيب نصي
                comparison = aValue.localeCompare(bValue, 'ar');
            }
            
            return newDirection === 'asc' ? comparison : -comparison;
        });
        
        // إزالة كلاسات الترتيب من جميع العناصر
        this.table.querySelectorAll('.sortable-header').forEach(h => {
            h.classList.remove('asc', 'desc');
        });
        
        // إضافة كلاس الترتيب للعنصر الحالي
        headerElement.classList.add(newDirection);
        
        // إعادة ترتيب الصفوف
        rows.forEach(row => tbody.appendChild(row));
        
        this.sortDirection[columnIndex] = newDirection;
    }


}


function parseNumber(str) {
    if (!str) return 0;
    return parseFloat(str.toString().replace(/,/g, '')) || 0;
}
// دالة لتنسيق العرض مع فاصل آلاف (دون أصفار إضافية)
function formatNumberWithSeparator(num) {
    return num.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
}
// دالة لتنسيق الإدخال أثناء الكتابة (إضافة فاصل الآلاف تلقائياً)
function formatNumberInput(input) {
    // تجاهل إذا كان المستخدم يختار نصاً أو يستخدم مفاتيح السهام
    const selection = window.getSelection().toString();
    if (selection !== '') return;
    if ([38, 40, 37, 39].includes(event.keyCode)) return;

    let value = input.value.replace(/[^\d]/g, ''); // إزالة غير الأرقام
    value = value ? parseInt(value, 10) : 0;
    input.value = value === 0 ? '' : formatNumberWithSeparator(value);
}
// Function to convert numbers to Arabic words
function numberToArabicWords(num) {
    if (num === 0) return 'صفر';
    if (num < 0) return 'سالب ' + numberToArabicWords(-num);
    let integerPart = Math.floor(num);
    const decimalPart = Math.round((num - integerPart) * 100);
    const words = convertIntegerToWords(integerPart);
    let result = words ? words + ' ريال' : '';
    if (decimalPart > 0) {
        const decimalWords = convertIntegerToWords(decimalPart);
        result += (result ? ' و' : '') + decimalWords + ' هللة';
    }
    return result;
}
function convertIntegerToWords(n) {
    const units = ["", "واحد", "اثنان", "ثلاثة", "أربعة", "خمسة", "ستة", "سبعة", "ثمانية", "تسعة", "عشرة", "أحد عشر", "اثنا عشر", "ثلاثة عشر", "أربعة عشر", "خمسة عشر", "ستة عشر", "سبعة عشر", "ثمانية عشر", "تسعة عشر"];
    const tens = ["", "", "عشرون", "ثلاثون", "أربعون", "خمسون", "ستون", "سبعون", "ثمانون", "تسعون"];
    const hundreds = ["", "مئة", "مئتان", "ثلاثمائة", "أربعمائة", "خمسمائة", "ستمائة", "سبعمائة", "ثمانمائة", "تسعمائة"];
    const groups = ["", "ألف", "مليون", "مليار", "تريليون"];
    if (n === 0) return '';
    if (n < 20) return units[n];
    if (n < 100) {
        const ten = Math.floor(n / 10);
        const unit = n % 10;
        return unit ? units[unit] + " و" + tens[ten] : tens[ten];
    }
    if (n < 1000) {
        const hundred = Math.floor(n / 100);
        const remainder = n % 100;
        return hundreds[hundred] + (remainder ? " و" + convertIntegerToWords(remainder) : "");
    }
    let word = "";
    let groupLevel = 0;
    while (n > 0) {
        const group = n % 1000;
        if (group) {
            let groupWord = convertIntegerToWords(group);
            if (groupLevel === 1) {
                if (group === 1) groupWord = "ألف";
                else if (group === 2) groupWord = "ألفان";
                else if (group > 2 && group < 11) groupWord += " آلاف";
                else groupWord += " ألف";
            } else if (groupLevel > 1) {
                if (group === 1) groupWord = groups[groupLevel];
                else if (group === 2) groupWord = groups[groupLevel] + "ان";
                else if (group > 2 && group < 11) groupWord += " " + groups[groupLevel] + "ات";
                else groupWord += " " + groups[groupLevel];
            }
            word = groupWord + (word ? " و" + word : "");
        }
        n = Math.floor(n / 1000);
        groupLevel++;
    }
    return word;
}
// Audio Context for sounds
let audioCtx = null;
function initAudio() {
    if (!audioCtx) {
        audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    }
}
function playSound(type) {
    initAudio();
    if (audioCtx.state === 'suspended') {
        audioCtx.resume();
    }
    const oscillator = audioCtx.createOscillator();
    const gainNode = audioCtx.createGain();
    oscillator.connect(gainNode);
    gainNode.connect(audioCtx.destination);
    let frequency = 440;
    let duration = 200;
    let typeWave = 'sine';
    switch (type) {
        case 'success':
            frequency = 800;
            duration = 150;
            typeWave = 'sine';
            break;
        case 'error':
            frequency = 400;
            duration = 300;
            typeWave = 'square';
            break;
        case 'warning':
            frequency = 600;
            duration = 250;
            typeWave = 'sawtooth';
            break;
        case 'click':
            frequency = 1000;
            duration = 50;
            typeWave = 'triangle';
            break;
        case 'confirm':
            frequency = 700;
            duration = 200;
            typeWave = 'sine';
            break;
    }
    oscillator.type = typeWave;
    oscillator.frequency.value = frequency;
    gainNode.gain.setValueAtTime(0.3, audioCtx.currentTime);
    gainNode.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + duration / 1000);
    oscillator.start(audioCtx.currentTime);
    oscillator.stop(audioCtx.currentTime + duration / 1000);
}
function checkPassword() {
    playSound('click');
    const password = document.getElementById('passwordInput').value;
    const savedPassword = localStorage.getItem('appPassword') || '123';
    if (password === savedPassword) {
        playSound('success');
        document.getElementById('passwordScreen').style.display = 'none';
        document.getElementById('mainContent').style.display = 'block';
        // تم نقل استدعاءات التهيئة إلى هنا عند نجاح الدخول
        initApp();
    } else {
        playSound('error');
        alert('كلمة المرور غير صحيحة! حاول مرة أخرى.');
    }
}

function togglePasswordProtection() {
    const isEnabled = document.getElementById('passwordToggle').checked;
    localStorage.setItem('passwordProtectionEnabled', isEnabled);
    document.getElementById('passwordManagementArea').style.display = isEnabled ? 'block' : 'none';
    playSound('click');
    if (isEnabled && !localStorage.getItem('appPassword')) {
        localStorage.setItem('appPassword', '123'); // كلمة المرور الافتراضية
        alert('تم تفعيل حماية كلمة المرور. كلمة المرور الافتراضية هي 123');
    }
}

function updateAppPassword() {
    const newPassword = document.getElementById('newAppPassword').value;
    if (newPassword.length < 1) {
        alert('يرجى إدخال كلمة مرور صالحة');
        return;
    }
    localStorage.setItem('appPassword', newPassword);
    document.getElementById('newAppPassword').value = '';
    playSound('success');
    alert('تم تحديث كلمة المرور بنجاح');
}

function initApp() {
    try { loadData(); } catch(e) { console.error("Error in loadData:", e); }
    try { applyPermissions(); } catch(e) { console.error("Error in applyPermissions:", e); }
    try { openTab('dashboard'); } catch(e) { console.error("Error in openTab:", e); }
    try { initCharts(); } catch(e) { console.error("Error in initCharts:", e); }
    try { updateHeader(); } catch(e) { console.error("Error in updateHeader:", e); }
    
    // تحديث حالة الواجهة في الإعدادات
    const isPasswordEnabled = localStorage.getItem('passwordProtectionEnabled') === 'true';
    const toggle = document.getElementById('passwordToggle');
    if (toggle) {
        toggle.checked = isPasswordEnabled;
        document.getElementById('passwordManagementArea').style.display = isPasswordEnabled ? 'block' : 'none';
    }
}
function clearLogo() {
    playSound('warning');
    if (confirm('هل أنت متأكد من مسح الشعار؟')) {
        localStorage.removeItem('logo');
        document.getElementById('logo').src = '';
        document.getElementById('logo').style.display = 'none';
        playSound('success');
        alert('تم مسح الشعار');
    }
}
function setClearPassword() {
    playSound('click');
    const clearPasswordInputSettings = document.getElementById('clearPasswordInputSettings');
    const clearPassword = clearPasswordInputSettings.value;
    if (clearPassword) {
        localStorage.setItem('clearPassword', clearPassword);
        playSound('success');
        alert('تم تعيين كلمة مرور المسح بنجاح');
        clearPasswordInputSettings.value = '';
    } else {
        playSound('error');
        alert('يرجى إدخال كلمة مرور');
    }
}
function openTab(tabName) {
    playSound('click');
    document.querySelectorAll('.tab-content').forEach(tab => {
        tab.classList.remove('active');
    });
    const tab = document.getElementById(tabName);
    if (tab) tab.classList.add('active');
    
    // تحديث البيانات تلقائياً عند فتح التبويبات
    if (tabName === 'workers') {
        loadWorkersTable();
    } else if (tabName === 'buildings') {
        loadBuildingsTable();
    } else if (tabName === 'dashboard') {
        initCharts();
    }
}
function clearInputs(inputs) {
    inputs.forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            if (element.tagName === 'INPUT') element.value = '';
            if (element.tagName === 'SELECT') element.selectedIndex = 0;
        }
    });
}
function toggleTable(tableId) {
    playSound('click');
    const table = document.getElementById(tableId);
    const button = event.target;
    if (table.style.display === 'none') {
        table.style.display = 'table';
        button.textContent = 'إخفاء الجدول';
    } else {
        table.style.display = 'none';
        button.textContent = 'إظهار الجدول';
    }
}
function toggleLists() {
    playSound('click');
    const lists = document.querySelector('.list-container');
    const button = event.target;
    if (lists.style.display === 'none') {
        lists.style.display = 'flex';
        button.textContent = 'إخفاء القوائم';
    } else {
        lists.style.display = 'none';
        button.textContent = 'إظهار القوائم';
    }
}
// --- IndexedDB Setup for Persistent Local Storage ---
const DB_NAME = 'AccountingDB';
const DB_VERSION = 1;
let db;

function initDB() {
    return new Promise((resolve) => {
        try {
            if (!window.indexedDB) {
                console.warn("IndexedDB not supported");
                return resolve(null);
            }
            const request = indexedDB.open(DB_NAME, DB_VERSION);
            
            // تحديد وقت زمني لفتح القاعدة (timeout)
            const timeout = setTimeout(() => {
                console.warn("IndexedDB open timeout");
                resolve(null);
            }, 2000);

            request.onupgradeneeded = (e) => {
                const db = e.target.result;
                const stores = ['workers', 'buildings', 'partnerships', 'settings', 'users', 'archived_buildings'];
                stores.forEach(store => {
                    if (!db.objectStoreNames.contains(store)) {
                        db.createObjectStore(store);
                    }
                });
            };
            request.onsuccess = (e) => {
                clearTimeout(timeout);
                db = e.target.result;
                console.log("IndexedDB Initialized");
                resolve(db);
            };
            request.onerror = (e) => {
                clearTimeout(timeout);
                console.error("IndexedDB Error:", e.target.error);
                resolve(null);
            };
        } catch (err) {
            console.error("Critical IndexedDB Error:", err);
            resolve(null);
        }
    });
}

function loadFromStorage(key) {
    const data = localStorage.getItem(key);
    return data ? JSON.parse(data) : [];
}

function saveToStorage(key, data) {
    // 1. Save to localStorage immediately (synchronous)
    localStorage.setItem(key, JSON.stringify(data));
    
    // 2. Backup to IndexedDB (asynchronous)
    if (db) {
        try {
            const tx = db.transaction(key, 'readwrite');
            const store = tx.objectStore(key);
            store.put(data, 'current_data');
        } catch (err) {
            console.error("Error saving to IndexedDB:", err);
        }
    }
}

async function syncFromIndexedDB() {
    if (!db) await initDB();
    if (!db) return;
    
    const stores = ['workers', 'buildings', 'partnerships', 'settings', 'users', 'archived_buildings'];
    for (const key of stores) {
        try {
            const tx = db.transaction(key, 'readonly');
            const store = tx.objectStore(key);
            const request = store.get('current_data');
            request.onsuccess = () => {
                if (request.result) {
                    localStorage.setItem(key, JSON.stringify(request.result));
                }
            };
        } catch (err) {
            console.error(`Error syncing ${key} from IndexedDB:`, err);
        }
    }
}

// Start DB Initialization
initDB().then(() => syncFromIndexedDB());
// --- End IndexedDB Setup ---
function getDayName(dateStr) {
    const days = ['الأحد', 'الإثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'];
    const date = new Date(dateStr);
    return days[date.getDay()];
}
function getDateTimeString(dateStr) {
    if (!dateStr) return '';
    const date = new Date(dateStr);
    if (isNaN(date.getTime())) return dateStr;
    const day = String(date.getDate()).padStart(2, '0');
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const year = date.getFullYear();
    const hours = date.getHours();
    const minutes = String(date.getMinutes()).padStart(2, '0');
    const ampm = hours >= 12 ? 'م' : 'ص';
    const displayHours = hours % 12 || 12;
    const timeFormatted = `${displayHours}:${minutes} ${ampm}`;
    const dateFormatted = `${day}/${month}/${year}`;
    // إرجاع التاريخ والوقت في سطرين منفصلين مع تنسيق CSS بسيط
    return `<div class="datetime-container">
                <span class="date-part">${dateFormatted}</span>
                <span class="time-part">${timeFormatted}</span>
            </div>`;
}
function updateHeader() {
    const now = new Date();
    const dayName = getDayName(now.toISOString().split('T')[0]);
    const date = now.toLocaleDateString('ar-EG');
    const time = now.toLocaleTimeString('ar-EG');
    document.getElementById('header').textContent = `${dayName} - ${date} - ${time}`;
}
function requireDeletePassword() {
    const clearPassword = localStorage.getItem('clearPassword');
    if (!clearPassword) {
        playSound('error');
        alert('لم يتم تعيين كلمة مرور الحذف من الإعدادات');
        return false;
    }
    const password = prompt('أدخل كلمة مرور الحذف');
    if (password !== clearPassword) {
        playSound('error');
        alert('كلمة مرور خاطئة');
        return false;
    }
    return true;
}
function addBuilding() {
    playSound('click');
    const name = document.getElementById('buildingName').value.trim();
    const location = document.getElementById('buildingLocation').value.trim();
    const note = document.getElementById('buildingNote').value.trim();
    if (!name || !location) {
        playSound('error');
        return alert('يرجى إدخال اسم العمارة والمكان');
    }
    const buildings = loadFromStorage('buildings');
    if (buildings.some(b => b.name.toLowerCase() === name.toLowerCase())) {
        playSound('error');
        return alert('اسم العمارة موجود بالفعل! يرجى اختيار اسم مختلف.');
    }
    if (!confirm('هل أنت متأكد؟')) return;
    buildings.push({ id: Date.now(), name, location, note: note || '', deliveries: [], isPaid: false });
    saveToStorage('buildings', buildings);
    loadBuildingsTable();
    loadBuildingLists();
    clearInputs(['buildingName', 'buildingLocation', 'buildingNote']);
    playSound('success');
    alert('تمت الإضافة بنجاح');
}
function togglePaid(buildingId) {
    playSound('confirm');
    let buildings = loadFromStorage('buildings');
    const building = buildings.find(b => b.id === buildingId);
    building.isPaid = !building.isPaid;
    saveToStorage('buildings', buildings);
    loadBuildingsTable();
    loadBuildingLists();
}
function deleteBuilding() {
    playSound('warning');
    const name = document.getElementById('buildingSelect').value;
    if (!name) {
        playSound('error');
        return alert('يرجى اختيار عمارة');
    }
    if (!requireDeletePassword()) return;
    if (!confirm('هل أنت متأكد؟')) return;
    let buildings = loadFromStorage('buildings');
    buildings = buildings.filter(b => b.name !== name);
    saveToStorage('buildings', buildings);
    loadBuildingsTable();
    loadBuildingLists();
    playSound('success');
    alert('تم الحذف بنجاح');
}
function addDelivery() {
    playSound('click');
    const building = document.getElementById('buildingSelect').value;
    const amount = parseNumber(document.getElementById('amountDelivered').value);
    let receiver = document.getElementById('receiver').value;
    const customReceiver = document.getElementById('customReceiver').value.trim();
    const date = document.getElementById('deliveryDate').value;
    const note = document.getElementById('deliveryNote').value.trim();
    if (!building || amount <= 0 || !date) {
        playSound('error');
        return alert('يرجى إدخال العمارة والمبلغ والتاريخ');
    }
    if (receiver === 'أخرى' && !customReceiver) {
        playSound('error');
        return alert('يرجى إدخال اسم المستلم');
    }
    if (!confirm('هل أنت متأكد؟')) return;
    receiver = receiver === 'أخرى' ? customReceiver : receiver;
    let buildings = loadFromStorage('buildings');
    const buildingData = buildings.find(b => b.name === building);
    const fullDate = new Date(`${date}T${new Date().toLocaleTimeString('en-US', {hour12: false})}`);
    buildingData.deliveries.push({ id: Date.now(), amount, receiver, date: fullDate.toISOString(), note: note || '' });
    saveToStorage('buildings', buildings);
    loadBuildingsTable();
    clearInputs(['buildingSelect', 'amountDelivered', 'receiver', 'customReceiver', 'deliveryDate', 'deliveryNote']);
    document.getElementById('customReceiver').style.display = 'none';
    playSound('success');
    alert('تمت الإضافة بنجاح');
}
function loadBuildingLists() {
    const buildings = loadFromStorage('buildings');
    const paidList = document.getElementById('paidList');
    const unpaidList = document.getElementById('unpaidList');
    if (!paidList || !unpaidList) return;
    
    paidList.innerHTML = '';
    unpaidList.innerHTML = '';
    buildings.forEach(b => {
        const li = document.createElement('li');
        li.style.display = 'flex';
        li.style.justifyContent = 'space-between';
        li.style.alignItems = 'center';
        li.style.padding = '8px';
        li.style.borderBottom = '1px solid var(--border-color)';
        
        const textSpan = document.createElement('span');
        textSpan.textContent = `${b.name} (${b.location})`;
        li.appendChild(textSpan);
        
        const buttonGroup = document.createElement('div');
        buttonGroup.style.display = 'flex';
        buttonGroup.style.gap = '5px';

        const viewBtn = document.createElement('button');
        viewBtn.className = 'btn btn-save';
        viewBtn.style.padding = '5px 10px';
        viewBtn.style.fontSize = '0.8rem';
        viewBtn.textContent = 'عرض';
        viewBtn.onclick = () => showBuildingDetailsInModal(b.id);
        buttonGroup.appendChild(viewBtn);

        const toggleBtn = document.createElement('button');
        toggleBtn.className = 'btn btn-paid';
        toggleBtn.style.padding = '5px 10px';
        toggleBtn.style.fontSize = '0.8rem';
        toggleBtn.textContent = b.isPaid ? 'إلغاء' : 'سدد';
        toggleBtn.onclick = () => togglePaid(b.id);
        buttonGroup.appendChild(toggleBtn);
        
        li.appendChild(buttonGroup);
        
        if (b.isPaid) {
            paidList.appendChild(li);
        } else {
            unpaidList.appendChild(li);
        }
    });
}

function showPaidBuildings() {
    const buildings = loadFromStorage('buildings').filter(b => b.isPaid);
    let content = '<div class="list-box"><ul>';
    if (buildings.length === 0) {
        content += '<li>لا توجد عماير مسددة حالياً</li>';
    } else {
        buildings.forEach(b => {
            content += `<li style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #eee;">
                <span>${b.name} (${b.location})</span>
                <button class="btn btn-save" style="padding:5px 15px;" onclick="showBuildingDetailsInModal('${b.id}')">عرض التفاصيل</button>
            </li>`;
        });
    }
    content += '</ul></div>';
    showModal('🏗️ العماير المسددة', content);
}

function showUnpaidBuildings() {
    const buildings = loadFromStorage('buildings').filter(b => !b.isPaid);
    let content = '<div class="list-box"><ul>';
    if (buildings.length === 0) {
        content += '<li>لا توجد عماير غير مسددة حالياً</li>';
    } else {
        buildings.forEach(b => {
            content += `<li style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #eee;">
                <span>${b.name} (${b.location})</span>
                <button class="btn btn-save" style="padding:5px 15px;" onclick="showBuildingDetailsInModal('${b.id}')">عرض التفاصيل</button>
            </li>`;
        });
    }
    content += '</ul></div>';
    showModal('🏗️ العماير غير المسددة', content);
}

function showBuildingDetailsInModal(buildingId) {
    console.log("Showing details for building ID:", buildingId);
    const buildings = loadFromStorage('buildings');
    // تحويل المعرف إلى نص أو رقم لضمان المطابقة
    const b = buildings.find(item => item.id == buildingId || item.id.toString() === buildingId.toString());
    
    if (!b) {
        console.error("Building not found!");
        return alert('تعذر العثور على بيانات هذه العمارة');
    }

    let total = b.deliveries.reduce((sum, d) => sum + parseNumber(d.amount), 0);
    
    // استخدام formatNumberWithSeparator بدلاً من formatNumber غير المعرفة
    let content = `
        <div style="margin-bottom:20px; padding:15px; background:var(--bg-color); border-radius:10px; border:1px solid var(--border-color);">
            <p><strong>اسم العمارة:</strong> ${b.name}</p>
            <p><strong>المكان:</strong> ${b.location}</p>
            <p><strong>الحالة:</strong> ${b.isPaid ? '<span style="color:var(--success-color)">✅ مسددة</span>' : '<span style="color:var(--danger-color)">❌ غير مسددة</span>'}</p>
            <p><strong>إجمالي المسلمات:</strong> <span style="color:var(--primary-color); font-weight:bold;">${formatNumberWithSeparator(total)} ريال</span></p>
            ${b.note ? `<p><strong>ملاحظة:</strong> ${b.note}</p>` : ''}
            <div style="margin-top:10px; display:flex; gap:10px;">
                <button class="btn btn-import" style="padding:5px 15px; font-size:0.9rem;" onclick="archiveBuilding('${b.id}')">📦 نقل للأرشيف</button>
            </div>
        </div>
        <h3>سجل المسلمات</h3>
        <div class="table-container">
            <table class="dynamic-table">
                <thead>
                    <tr>
                        <th>المبلغ</th>
                        <th>المستلم</th>
                        <th>التاريخ</th>
                        <th>ملاحظة</th>
                    </tr>
                </thead>
                <tbody>
                    ${b.deliveries.length > 0 ? b.deliveries.map(d => `
                        <tr>
                            <td>${formatNumberWithSeparator(parseNumber(d.amount))}</td>
                            <td>${d.receiver}</td>
                            <td>${getDateTimeString(d.date)}</td>
                            <td>${d.note || '-'}</td>
                        </tr>
                    `).join('') : '<tr><td colspan="4" style="text-align:center;">لا توجد مسلمات مسجلة</td></tr>'}
                </tbody>
            </table>
        </div>
    `;
    
    showModal(`تفاصيل العمارة: ${b.name}`, content);
}
function loadBuildingsTable() {
    const buildings = loadFromStorage('buildings');
    const selects = [
        'buildingSelect', 'buildingReportSelect', 'workBuilding',
        'yahyaReceivedFrom', 'saddamReceivedFrom'
    ];
    selects.forEach(id => {
        const select = document.getElementById(id);
        if (select) select.innerHTML = '<option value="">اختر عمارة</option>' + buildings.filter(b => !b.isPaid).map(b => `<option value="${b.name}">${b.name}</option>`).join('');
    });
    const table = document.getElementById('buildingsTable').getElementsByTagName('tbody')[0];
    table.innerHTML = '';
    buildings.forEach(b => {
        if (b.deliveries.length > 0) {
            b.deliveries.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
            const total = b.deliveries.reduce((sum, d) => sum + d.amount, 0);
            b.deliveries.forEach((d, i) => {
                const dayName = getDayName(d.date);
                const dateTimeStr = getDateTimeString(d.date);
                const row = table.insertRow();
	                row.innerHTML = `
	                    <td>${b.name}<br><span class="total-text">${b.location}</span></td>
	                    <td>${formatNumberWithSeparator(d.amount)}</td>
	                    <td>${d.receiver}</td>
	                    <td>${dateTimeStr} (${dayName})</td>
	                    <td>${d.note}</td>
	                    <td>${i === 0 ? formatNumberWithSeparator(total) + '<br><span class="total-text">ريال</span><br><span class="total-text">' + numberToArabicWords(total) + '</span>' : ''}</td>
	                    <td>${b.isPaid ? '✅' : '❌'}</td>
	                    <td>
	                        <button class="btn btn-info" onclick="openEditModal('delivery', '${b.id}', '${d.id}')">تعديل</button>
	                        <button class="btn btn-delete" onclick="deleteDelivery('${b.id}', '${d.id}')">حذف</button>
	                        <button class="btn btn-paid" onclick="togglePaid(${b.id})">${b.isPaid ? 'إلغاء الدفع' : 'تحديد كمسدد'}</button>
	                    </td>
	                `;
            });
        } else {
            const row = table.insertRow();
            row.innerHTML = `
                <td>${b.name}<br><span class="total-text">${b.location}</span></td>
                <td colspan="4">لا توجد مسلمات</td>
                <td>0<br><span class="total-text">ريال</span><br><span class="total-text">${numberToArabicWords(0)}</span></td>
                <td>${b.isPaid ? '✅' : '❌'}</td>
                <td>
                    <button class="btn btn-paid" onclick="togglePaid(${b.id})">${b.isPaid ? 'إلغاء الدفع' : 'تحديد كمسدد'}</button>
                </td>
            `;
        }
    });
}

function deleteDelivery(buildingId, deliveryId) {
    playSound('warning');
    if (!confirm('هل أنت متأكد؟')) return;
    let buildings = loadFromStorage('buildings');
    const building = buildings.find(b => b.id === parseInt(buildingId));
    let deliveryIndex = building.deliveries.findIndex(d => d.id && d.id.toString() === deliveryId.toString());
    if (deliveryIndex === -1) {
        const index = parseInt(deliveryId);
        if (!isNaN(index) && index >= 0 && index < building.deliveries.length) {
            deliveryIndex = index;
        }
    }
    if (deliveryIndex !== -1) {
        building.deliveries.splice(deliveryIndex, 1);
        saveToStorage('buildings', buildings);
        loadBuildingsTable();
        playSound('success');
    } else {
        alert('تعذر العثور على السجل المطلوب حذفه');
    }
}
function generateBuildingReport() {
    playSound('click');
    const building = document.getElementById('buildingReportSelect').value;
    if (!building) {
        playSound('error');
        return alert('يرجى اختيار عمارة');
    }
    const buildings = loadFromStorage('buildings');
    const buildingData = buildings.find(b => b.name === building);
    if (!buildingData.deliveries.length) {
        playSound('error');
        return alert('لا توجد بيانات');
    }
    buildingData.deliveries.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
    const total = buildingData.deliveries.reduce((sum, d) => sum + d.amount, 0);
    let report = `<h3>تقرير ${building}<br><span class="total-text">المكان: ${buildingData.location}</span></h3>
    <div style="background: rgba(2, 136, 209, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--primary-color);">
        <p style="margin: 0 0 10px 0; font-weight: bold; color: var(--primary-color);">📊 الإجمالي</p>
        <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--primary-color);">${formatNumberWithSeparator(total)}</p>
        <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.9rem;">ريال</p>
        <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.85rem;">${numberToArabicWords(total)}</p>
    </div>
    <table><thead><tr><th>المبلغ المسلم</th><th>المستلم</th><th>التاريخ (اليوم)</th><th>ملاحظة</th></tr></thead><tbody>`;
    buildingData.deliveries.forEach(d => {
        const dayName = getDayName(d.date);
        const dateTimeStr = getDateTimeString(d.date);
        report += `<tr><td>${formatNumberWithSeparator(d.amount)}</td><td>${d.receiver}</td><td><div style="display:flex; flex-direction:column; align-items:center;">${dateTimeStr}<span style="font-size:0.75rem; color:var(--nav-text); margin-top:2px;">(${dayName})</span></div></td><td>${d.note}</td></tr>`;
    });
    report += `</tbody></table>`;
    document.getElementById('buildingReport').innerHTML = report;
    showModal('تقرير العمارة', report, saveBuildingReportAsPDF);
    playSound('success');
}
function addWorker() {
    playSound('click');
    const name = document.getElementById('workerName').value.trim();
    const wage = parseNumber(document.getElementById('workerWage').value);
    const note = document.getElementById('workerNote').value.trim();
    if (!name || wage <= 0) {
        playSound('error');
        return alert('يرجى إدخال اسم العامل وأجر صالح');
    }
    const workers = loadFromStorage('workers');
    if (workers.some(w => w.name.toLowerCase() === name.toLowerCase())) {
        playSound('error');
        return alert('اسم العامل موجود بالفعل! يرجى اختيار اسم مختلف.');
    }
    if (!confirm('هل أنت متأكد؟')) return;
    workers.push({ id: Date.now(), name, wage, note: note || '', work: [], startDate: '' });
    saveToStorage('workers', workers);
    loadWorkersTable();
    clearInputs(['workerName', 'workerWage', 'workerNote']);
    playSound('success');
    alert('تمت الإضافة بنجاح');
}
function deleteWorker() {
    playSound('warning');
    const name = document.getElementById('workerSelect').value;
    if (!name) {
        playSound('error');
        return alert('يرجى اختيار عامل');
    }
    if (!requireDeletePassword()) return;
    if (!confirm('هل أنت متأكد؟')) return;
    let workers = loadFromStorage('workers');
    workers = workers.filter(w => w.name !== name);
    saveToStorage('workers', workers);
    loadWorkersTable();
    playSound('success');
    alert('تم الحذف بنجاح');
}
function addWork() {
    playSound('click');
    const worker = document.getElementById('workerSelect').value;
    const period = parseFloat(document.getElementById('workPeriod').value);
    const delivered = parseNumber(document.getElementById('workerDelivered').value);
    const extra = parseNumber(document.getElementById('workerExtra').value);
    const transport = parseNumber(document.getElementById('workerTransport').value);
    const date = document.getElementById('workDate').value;
    const building = document.getElementById('workBuilding').value;
    const note = document.getElementById('workNote').value.trim();
    if (!worker || isNaN(period) || period < 0 || !date || !building) {
        playSound('error');
        return alert('يرجى إدخال البيانات بشكل صحيح');
    }
    if (!confirm('هل أنت متأكد؟')) return;
    let workers = loadFromStorage('workers');
    const workerData = workers.find(w => w.name === worker);
    const daily = workerData.wage * period;
    const remaining = daily + extra + transport - delivered;
    const fullDate = new Date(`${date}T${new Date().toLocaleTimeString('en-US', {hour12: false})}`);
    workerData.work.push({ id: Date.now(), period, delivered, extra, transport, date: fullDate.toISOString(), building, daily, remaining, note: note || '' });
    saveToStorage('workers', workers);
    loadWorkersTable();
    clearInputs(['workerSelect', 'workPeriod', 'workerDelivered', 'workerExtra', 'workerTransport', 'workDate', 'workBuilding', 'workNote']);
    playSound('success');
    alert('تمت الإضافة بنجاح');
}
function loadWorkersTable() {
    const workers = loadFromStorage('workers') || [];
    const selects = ['workerSelect', 'workerReportSelect'];
    selects.forEach(id => {
        const select = document.getElementById(id);
        if (select) {
            const currentValue = select.value;
            select.innerHTML = '<option value="">اختر عامل</option>' + workers.map(w => `<option value="${w.name}">${w.name}</option>`).join('');
            if (currentValue && workers.some(w => w.name === currentValue)) {
                select.value = currentValue;
            }
        }
    });
    const table = document.getElementById('workersTable').getElementsByTagName('tbody')[0];
    table.innerHTML = '';
    let totalRemaining = 0;
    
    if (workers.length === 0) {
        const row = table.insertRow();
        row.innerHTML = '<td colspan="11" style="text-align:center;">لا يوجد عمال مسجلين</td>';
    } else {
        workers.forEach(w => {
            let workerTotalRemaining = 0;
            const workList = w.work || [];
            
            if (workList.length > 0) {
                workList.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
                workList.forEach((wk, i) => {
                    workerTotalRemaining += (Number(wk.remaining) || 0);
                    const dayName = getDayName(wk.date);
                    const dateTimeStr = getDateTimeString(wk.date);
                    const row = table.insertRow();
	                    row.innerHTML = `
	                        <td>${w.name}</td>
	                        <td>${wk.period}</td>
	                        <td>${formatNumberWithSeparator(wk.delivered)}</td>
	                        <td>${formatNumberWithSeparator(wk.extra)}</td>
	                        <td>${formatNumberWithSeparator(wk.transport)}</td>
	                        <td>${dateTimeStr} (${dayName})</td>
	                        <td>${wk.building}</td>
	                        <td>${formatNumberWithSeparator(wk.daily)}</td>
	                        <td>${formatNumberWithSeparator(wk.remaining)}</td>
	                        <td>${wk.note || ''}</td>
	                        <td>
	                            <button class="btn btn-info" onclick="openEditModal('work', '${w.id}', '${wk.id}')">تعديل</button>
	                            <button class="btn btn-delete" onclick="deleteWork('${w.id}', '${wk.id}')">حذف</button>
	                        </td>
	                    `;
                });
            } else {
                // عرض العامل حتى لو لم يكن لديه سجل عمل
                const row = table.insertRow();
                row.innerHTML = `
                    <td>${w.name}</td>
                    <td colspan="8" style="text-align:center; color:var(--nav-text)">لا توجد سجلات عمل لهذا العامل</td>
                    <td>${w.note || ''}</td>
                    <td>-</td>
                `;
            }
            totalRemaining += workerTotalRemaining;
        });
    }
    
    const totalValElem = document.getElementById('totalRemainingValue');
    const totalWordsElem = document.getElementById('totalRemainingWords');
    if (totalValElem) totalValElem.textContent = formatNumberWithSeparator(totalRemaining);
    if (totalWordsElem) totalWordsElem.textContent = numberToArabicWords(totalRemaining);

    // تحديث جدول العمال الأساسي (workersListTable)
    const listTable = document.getElementById('workersListTable');
    if (listTable) {
        const tbody = listTable.getElementsByTagName('tbody')[0];
        tbody.innerHTML = '';
        workers.forEach(w => {
            const row = tbody.insertRow();
            row.innerHTML = `
                <td>${w.name}</td>
                <td>${formatNumberWithSeparator(w.wage)}</td>
                <td>${w.note || ''}</td>
                <td>
                    <button class="btn btn-info" onclick="openEditModal('worker', '${w.name}', null)">تعديل</button>
                    <button class="btn btn-delete" onclick="deleteWorkerByName('${w.name}')">حذف</button>
                </td>
            `;
        });
    }
}

// دالة مساعدة لحذف عامل بالاسم من جدول العمال الأساسي
function deleteWorkerByName(name) {
    playSound('warning');
    if (!requireDeletePassword()) return;
    if (!confirm(`هل أنت متأكد من حذف العامل "${name}"؟`)) return;
    let workers = loadFromStorage('workers');
    workers = workers.filter(w => w.name !== name);
    saveToStorage('workers', workers);
    loadWorkersTable();
    playSound('success');
    alert('تم الحذف بنجاح');
}

function deleteWork(workerId, workId) {
    playSound('warning');
    if (!confirm('هل أنت متأكد؟')) return;
    let workers = loadFromStorage('workers');
    const worker = workers.find(w => w.id === parseInt(workerId));
    let workIndex = worker.work.findIndex(wk => wk.id && wk.id.toString() === workId.toString());
    if (workIndex === -1) {
        const index = parseInt(workId);
        if (!isNaN(index) && index >= 0 && index < worker.work.length) {
            workIndex = index;
        }
    }
    if (workIndex !== -1) {
        worker.work.splice(workIndex, 1);
        saveToStorage('workers', workers);
        loadWorkersTable();
        playSound('success');
    } else {
        alert('تعذر العثور على السجل المطلوب حذفه');
    }
}
function generateWorkerReport() {
    playSound('click');
    const workerSelect = document.getElementById('workerReportSelect');
    if (!workerSelect) return;
    
    const worker = workerSelect.value;
    if (!worker) {
        playSound('error');
        return alert('يرجى اختيار عامل');
    }
    
    const workers = loadFromStorage('workers') || [];
    const workerData = workers.find(w => w.name === worker);
    
    if (!workerData) {
        playSound('error');
        return alert('تعذر العثور على بيانات العامل');
    }
    
    const workList = workerData.work || [];
    if (workList.length === 0) {
        playSound('error');
        return alert('لا توجد بيانات عمل مسجلة لهذا العامل');
    }
    
    let startDateInput = document.getElementById('workerReportStart').value;
    let endDateInput = document.getElementById('workerReportEnd').value;
    const savedStartDate = workerData.startDate;
    
    if (!startDateInput && savedStartDate) {
        startDateInput = savedStartDate;
        document.getElementById('workerReportStart').value = savedStartDate;
    }
    
    if (!endDateInput) {
        endDateInput = new Date().toISOString().split('T')[0];
        document.getElementById('workerReportEnd').value = endDateInput;
    }
    
    let filteredWork = [...workList];
    if (startDateInput && endDateInput) {
        filteredWork = filteredWork.filter(w => w.date.split('T')[0] >= startDateInput && w.date.split('T')[0] <= endDateInput);
    }
    
    filteredWork.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
    
    if (filteredWork.length === 0) {
        playSound('error');
        return alert('لا توجد بيانات في الفترة المحددة');
    }
    
    const totalRemaining = filteredWork.reduce((sum, w) => sum + (Number(w.remaining) || 0), 0);
    
    let report = `<h3>تقرير ${worker}</h3>
    <div style="background: rgba(229, 57, 53, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--danger-color);">
        <p style="margin: 0 0 10px 0; font-weight: bold; color: var(--danger-color);">💰 الإجمالي الباقي</p>
        <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--danger-color);">${formatNumberWithSeparator(totalRemaining)}</p>
        <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.9rem;">ريال</p>
        <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.85rem;">${numberToArabicWords(totalRemaining)}</p>
    </div>
    <div class="table-container">
        <table id="workerReportTable" class="dynamic-table">
            <thead>
                <tr>
                    <th>فترة العمل</th>
                    <th>المسلم</th>
                    <th>الإضافي</th>
                    <th>المواصلات</th>
                    <th>التاريخ (اليوم)</th>
                    <th>مكان العمل</th>
                    <th>اليومية</th>
                    <th>الباقي</th>
                    <th>ملاحظة</th>
                </tr>
            </thead>
            <tbody>`;
            
    filteredWork.forEach(w => {
        const dayName = getDayName(w.date);
        const dateTimeStr = getDateTimeString(w.date);
	        report += `<tr>
	            <td>${w.period}</td>
	            <td>${formatNumberWithSeparator(w.delivered)}</td>
	            <td>${formatNumberWithSeparator(w.extra)}</td>
	            <td>${formatNumberWithSeparator(w.transport)}</td>
	            <td><div style="display:flex; flex-direction:column; align-items:center;">${dateTimeStr}<span style="font-size:0.75rem; color:var(--nav-text); margin-top:2px;">(${dayName})</span></div></td>
	            <td>${w.building}</td>
	            <td>${formatNumberWithSeparator(w.daily)}</td>
	            <td>${formatNumberWithSeparator(w.remaining)}</td>
	            <td>${w.note || ''}</td>
	        </tr>`;
    });
    
    report += `</tbody>
        </table>
    </div>`;
    
    const reportDiv = document.getElementById('workerReport');
    if (reportDiv) {
        reportDiv.innerHTML = report;
        reportDiv.style.display = 'block';
    }
    
    showModal('تقرير العامل', report, saveWorkerReportAsPDF);
    playSound('success');
}
function loadWorkerStartDate(workerName) {
    if (!workerName) return;
    const workers = loadFromStorage('workers');
    const worker = workers.find(w => w.name === workerName);
    if (worker && worker.startDate) {
        document.getElementById('workerReportStart').value = worker.startDate;
    }
}
function saveWorkerStartDate() {
    playSound('click');
    const workerName = document.getElementById('workerReportSelect').value;
    const startDate = document.getElementById('workerReportStart').value;
    if (!workerName || !startDate) {
        playSound('error');
        return alert('يرجى اختيار عامل وتاريخ');
    }
    let workers = loadFromStorage('workers');
    const worker = workers.find(w => w.name === workerName);
    worker.startDate = startDate;
    saveToStorage('workers', workers);
    playSound('success');
    alert('تم حفظ تاريخ البداية');
}
function addPartnership() {
    playSound('click');
    const yahyaReceived = parseNumber(document.getElementById('yahyaReceived').value);
    const yahyaReceivedFrom = document.getElementById('yahyaReceivedFrom').value;
    const yahyaPaidWorkers = parseNumber(document.getElementById('yahyaPaidWorkers').value);
    const yahyaExpenses = parseNumber(document.getElementById('yahyaExpenses').value);
    const yahyaToSaddam = parseNumber(document.getElementById('yahyaToSaddam').value);
    const saddamReceived = parseNumber(document.getElementById('saddamReceived').value);
    const saddamReceivedFrom = document.getElementById('saddamReceivedFrom').value;
    const saddamPaidWorkers = parseNumber(document.getElementById('saddamPaidWorkers').value);
    const saddamExpenses = parseNumber(document.getElementById('saddamExpenses').value);
    const saddamToYahya = parseNumber(document.getElementById('saddamToYahya').value);
    const date = document.getElementById('partnershipDate').value;
    const note = document.getElementById('partnershipNote').value.trim();
    if (!date) {
        playSound('error');
        return alert('يرجى إدخال التاريخ');
    }
    if (!confirm('هل أنت متأكد؟')) return;
    let partnerships = loadFromStorage('partnerships');
    const fullDate = new Date(`${date}T${new Date().toLocaleTimeString('en-US', {hour12: false})}`);
    partnerships.push({ id: Date.now(), yahyaReceived, yahyaReceivedFrom, yahyaPaidWorkers, yahyaExpenses, yahyaToSaddam, saddamReceived, saddamReceivedFrom, saddamPaidWorkers, saddamExpenses, saddamToYahya, date: fullDate.toISOString(), note: note || '' });
    saveToStorage('partnerships', partnerships);
    loadPartnershipTable();
    clearInputs(['yahyaReceived', 'yahyaReceivedFrom', 'yahyaPaidWorkers', 'yahyaExpenses', 'yahyaToSaddam', 'saddamReceived', 'saddamReceivedFrom', 'saddamPaidWorkers', 'saddamExpenses', 'saddamToYahya', 'partnershipDate', 'partnershipNote']);
    playSound('success');
    alert('تمت الإضافة بنجاح');
}
function loadPartnershipTable() {
    const partnerships = loadFromStorage('partnerships');
    partnerships.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
    const table = document.getElementById('partnershipTable').getElementsByTagName('tbody')[0];
    table.innerHTML = '';
    partnerships.forEach((d, i) => {
        const dayName = getDayName(d.date);
        const dateTimeStr = getDateTimeString(d.date);
        const row = table.insertRow();
	        row.innerHTML = `
	            <td><div style="display:flex; flex-direction:column; align-items:center;">${dateTimeStr}<span style="font-size:0.75rem; color:var(--nav-text); margin-top:2px;">(${dayName})</span></div></td>
	            <td>${formatNumberWithSeparator(d.yahyaReceived)}</td>
            <td>${d.yahyaReceivedFrom}</td>
            <td>${formatNumberWithSeparator(d.yahyaPaidWorkers)}</td>
            <td>${formatNumberWithSeparator(d.yahyaExpenses)}</td>
            <td>${formatNumberWithSeparator(d.yahyaToSaddam)}</td>
            <td>${formatNumberWithSeparator(d.saddamReceived)}</td>
            <td>${d.saddamReceivedFrom}</td>
            <td>${formatNumberWithSeparator(d.saddamPaidWorkers)}</td>
            <td>${formatNumberWithSeparator(d.saddamExpenses)}</td>
            <td>${formatNumberWithSeparator(d.saddamToYahya)}</td>
            <td>${d.note}</td>
            <td>
                <button class="btn btn-info" onclick="openEditModal('partnership', '${d.id}', null)">تعديل</button>
                <button class="btn btn-delete" onclick="deletePartnership(${d.id})">حذف</button>
            </td>
        `;
    });
    const startDate = localStorage.getItem('partnershipStartDate');
    if (startDate) document.getElementById('partnershipReportStart').value = startDate;
    
    // تحديث ملخص التصفية إذا كان معروضاً
    const settlementDiv = document.getElementById('settlement');
    if (settlementDiv && settlementDiv.style.display !== 'none') {
        settlePartnership();
    }
}

function deletePartnership(id) {
    playSound('warning');
    if (!confirm('هل أنت متأكد؟')) return;
    let partnerships = loadFromStorage('partnerships');
    partnerships = partnerships.filter(d => d.id !== id);
    saveToStorage('partnerships', partnerships);
    loadPartnershipTable();
    playSound('success');
}
function settlePartnership() {
    playSound('click');
    const partnerships = loadFromStorage('partnerships');
    if (!partnerships.length) {
        playSound('error');
        return alert('لا توجد بيانات');
    }
    let yahyaTotalReceived = 0, yahyaTotalPaidWorkers = 0, yahyaTotalExpenses = 0, yahyaToSaddam = 0;
    let saddamTotalReceived = 0, saddamTotalPaidWorkers = 0, saddamTotalExpenses = 0, saddamToYahya = 0;
    partnerships.forEach(d => {
        yahyaTotalReceived += d.yahyaReceived;
        yahyaTotalPaidWorkers += d.yahyaPaidWorkers;
        yahyaTotalExpenses += d.yahyaExpenses;
        yahyaToSaddam += d.yahyaToSaddam;
        saddamTotalReceived += d.saddamReceived;
        saddamTotalPaidWorkers += d.saddamPaidWorkers;
        saddamTotalExpenses += d.saddamExpenses;
        saddamToYahya += d.saddamToYahya;
    });
    const yahyaRemaining = yahyaTotalReceived - yahyaTotalPaidWorkers - yahyaTotalExpenses;
    const saddamRemaining = saddamTotalReceived - saddamTotalPaidWorkers - saddamTotalExpenses;
    const total = yahyaRemaining + saddamRemaining;
    const yahyaShare = (total / 2) - yahyaRemaining - saddamToYahya + yahyaToSaddam;
    const saddamShare = (total / 2) - saddamRemaining - yahyaToSaddam + saddamToYahya;
    const settlementContent = `
        <h3 style="text-align: center; color: var(--primary-color); margin-bottom: 20px;">📊 ملخص تصفية الحساب</h3>
        <div style="background: var(--bg-color); padding: 15px; border-radius: 10px; margin-bottom: 15px;">
            <h4 style="color: var(--primary-color); margin-top: 0;">📋 البيانات المالية:</h4>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div style="background: var(--card-bg); padding: 12px; border-radius: 8px; border-right: 4px solid #ff9800;">
                    <p style="margin: 0 0 5px 0; font-size: 0.9rem; color: var(--nav-text);">الباقي مع يحيى</p>
                    <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--primary-color);">${formatNumberWithSeparator(yahyaRemaining)} ريال</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${numberToArabicWords(yahyaRemaining)}</p>
                </div>
                <div style="background: var(--card-bg); padding: 12px; border-radius: 8px; border-right: 4px solid #2196f3;">
                    <p style="margin: 0 0 5px 0; font-size: 0.9rem; color: var(--nav-text);">الباقي مع صدام</p>
                    <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--primary-color);">${formatNumberWithSeparator(saddamRemaining)} ريال</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${numberToArabicWords(saddamRemaining)}</p>
                </div>
            </div>
        </div>
        <div style="background: var(--bg-color); padding: 15px; border-radius: 10px; margin-bottom: 15px;">
            <h4 style="color: var(--primary-color); margin-top: 0;">💰 الإجمالي والتوزيع:</h4>
            <div style="background: var(--card-bg); padding: 12px; border-radius: 8px; margin-bottom: 12px; border-right: 4px solid var(--success-color);">
                <p style="margin: 0 0 5px 0; font-size: 0.9rem; color: var(--nav-text);">الكل</p>
                <p style="margin: 0; font-size: 1.4rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(total)} ريال</p>
                <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${numberToArabicWords(total)}</p>
            </div>
        </div>
        <div style="background: var(--bg-color); padding: 15px; border-radius: 10px;">
            <h4 style="color: var(--primary-color); margin-top: 0;">🤝 النصيب النهائي:</h4>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div style="background: var(--card-bg); padding: 12px; border-radius: 8px; border-right: 4px solid ${yahyaShare >= 0 ? '#4caf50' : '#f44336'};">
                    <p style="margin: 0 0 5px 0; font-size: 0.9rem; color: var(--nav-text);">نصيب يحيى</p>
                    <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: ${yahyaShare >= 0 ? '#4caf50' : '#f44336'};">${formatNumberWithSeparator(yahyaShare)} ريال</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${yahyaShare >= 0 ? '✅ يقبض' : '❌ يدفع'}</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${numberToArabicWords(Math.abs(yahyaShare))}</p>
                </div>
                <div style="background: var(--card-bg); padding: 12px; border-radius: 8px; border-right: 4px solid ${saddamShare >= 0 ? '#4caf50' : '#f44336'};">
                    <p style="margin: 0 0 5px 0; font-size: 0.9rem; color: var(--nav-text);">نصيب صدام</p>
                    <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: ${saddamShare >= 0 ? '#4caf50' : '#f44336'};">${formatNumberWithSeparator(saddamShare)} ريال</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${saddamShare >= 0 ? '✅ يقبض' : '❌ يدفع'}</p>
                    <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: var(--nav-text);">${numberToArabicWords(Math.abs(saddamShare))}</p>
                </div>
            </div>
        </div>
    `;
    document.getElementById('settlement').innerHTML = settlementContent;
    showModal('📋 ملخص تصفية الحساب', settlementContent);
    playSound('success');
}
function generatePartnershipReport() {
    playSound('click');
    const startDate = document.getElementById('partnershipReportStart').value;
    const endDate = document.getElementById('partnershipReportEnd').value;
    let partnerships = loadFromStorage('partnerships');
    if (!partnerships.length) {
        playSound('error');
        return alert('لا توجد بيانات');
    }
    if (startDate && endDate) partnerships = partnerships.filter(d => d.date.split('T')[0] >= startDate && d.date.split('T')[0] <= endDate);
    partnerships.sort((a, b) => currentSortOrder === "desc" ? new Date(b.date) - new Date(a.date) : new Date(a.date) - new Date(b.date));
    if (!partnerships.length) {
        playSound('error');
        return alert('لا توجد بيانات في الفترة المحددة');
    }
    let yahyaTotalReceived = 0, yahyaTotalPaidWorkers = 0, yahyaTotalExpenses = 0, yahyaToSaddam = 0;
    let saddamTotalReceived = 0, saddamTotalPaidWorkers = 0, saddamTotalExpenses = 0, saddamToYahya = 0;
    partnerships.forEach(d => {
        yahyaTotalReceived += d.yahyaReceived;
        yahyaTotalPaidWorkers += d.yahyaPaidWorkers;
        yahyaTotalExpenses += d.yahyaExpenses;
        yahyaToSaddam += d.yahyaToSaddam;
        saddamTotalReceived += d.saddamReceived;
        saddamTotalPaidWorkers += d.saddamPaidWorkers;
        saddamTotalExpenses += d.saddamExpenses;
        saddamToYahya += d.saddamToYahya;
    });
    const yahyaRemaining = yahyaTotalReceived - yahyaTotalPaidWorkers - yahyaTotalExpenses;
    const saddamRemaining = saddamTotalReceived - saddamTotalPaidWorkers - saddamTotalExpenses;
    const total = yahyaRemaining + saddamRemaining;
    const yahyaShare = (total / 2) - yahyaRemaining - saddamToYahya + yahyaToSaddam;
    const saddamShare = (total / 2) - saddamRemaining - yahyaToSaddam + saddamToYahya;
    let report = `<h3>تقرير الشراكة</h3>
    <div style="background: rgba(67, 160, 71, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--success-color);">
        <p style="margin: 0 0 15px 0; font-weight: bold; color: var(--success-color);">📊 ملخص الأرصدة والنصيب</p>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div>
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الباقي مع يحيى</p>
                <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(yahyaRemaining)}</p>
            </div>
            <div>
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الباقي مع صدام</p>
                <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(saddamRemaining)}</p>
            </div>
            <div>
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الإجمالي</p>
                <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(total)}</p>
            </div>
            <div>
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الكل</p>
                <p style="margin: 0; font-size: 0.9rem; color: var(--nav-text);">${numberToArabicWords(total)}</p>
            </div>
        </div>
    </div>
    <div style="background: rgba(244, 67, 54, 0.05); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--danger-color);">
        <p style="margin: 0 0 15px 0; font-weight: bold; color: var(--primary-color);">💰 النصيب النهائي</p>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div style="background: var(--card-bg); padding: 15px; border-radius: 8px;">
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">نصيب يحيى</p>
                <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: ${yahyaShare >= 0 ? 'var(--success-color)' : 'var(--danger-color)'};">${formatNumberWithSeparator(yahyaShare)}</p>
                <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: ${yahyaShare >= 0 ? 'var(--success-color)' : 'var(--danger-color)'};">${yahyaShare >= 0 ? '✅ يقبض' : '❌ يدفع'}</p>
            </div>
            <div style="background: var(--card-bg); padding: 15px; border-radius: 8px;">
                <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">نصيب صدام</p>
                <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: ${saddamShare >= 0 ? 'var(--success-color)' : 'var(--danger-color)'};">${formatNumberWithSeparator(saddamShare)}</p>
                <p style="margin: 5px 0 0 0; font-size: 0.85rem; color: ${saddamShare >= 0 ? 'var(--success-color)' : 'var(--danger-color)'};">${saddamShare >= 0 ? '✅ يقبض' : '❌ يدفع'}</p>
            </div>
        </div>
    </div>
    <table><thead><tr><th>التاريخ (اليوم)</th><th>قبض يحيى</th><th>من</th><th>دفع للعمال</th><th>صرفة</th><th>المسلم مع صدام</th><th>قبض صدام</th><th>من</th><th>دفع للعمال</th><th>صرفة</th><th>المسلم مع يحيى</th><th>ملاحظة</th></tr></thead><tbody>`;
    partnerships.forEach(d => {
        const dayName = getDayName(d.date);
        const dateTimeStr = getDateTimeString(d.date);
        report += `<tr><td>${dateTimeStr} (${dayName})</td><td>${formatNumberWithSeparator(d.yahyaReceived)}</td><td>${d.yahyaReceivedFrom}</td><td>${formatNumberWithSeparator(d.yahyaPaidWorkers)}</td><td>${formatNumberWithSeparator(d.yahyaExpenses)}</td><td>${formatNumberWithSeparator(d.yahyaToSaddam)}</td><td>${formatNumberWithSeparator(d.saddamReceived)}</td><td>${d.saddamReceivedFrom}</td><td>${formatNumberWithSeparator(d.saddamPaidWorkers)}</td><td>${formatNumberWithSeparator(d.saddamExpenses)}</td><td>${formatNumberWithSeparator(d.saddamToYahya)}</td><td>${d.note}</td></tr>`;
    });
    report += `</tbody></table>`;
    document.getElementById('settlement').innerHTML = report;
    showModal('تقرير تصفية الشراكة', report, savePartnershipReportAsPDF);
    playSound('success');
}
function savePartnershipStartDate() {
    playSound('click');
    const startDate = document.getElementById('partnershipReportStart').value;
    if (!startDate) {
        playSound('error');
        return alert('يرجى إدخال تاريخ');
    }
    localStorage.setItem('partnershipStartDate', startDate);
    playSound('success');
    alert('تم حفظ تاريخ البداية');
}
function clearBuildingsData() {
    playSound('warning');
    alert('تحذير: سيتم مسح جميع بيانات العماير!');
    alert('تحذير ثاني: هذا الإجراء لا يمكن التراجع عنه!');
    if (!confirm('هل أنت متأكد؟')) return;
    const password = prompt('أدخل كلمة مرور المسح');
    if (password !== localStorage.getItem('clearPassword')) {
        playSound('error');
        return alert('كلمة مرور خاطئة');
    }
    localStorage.removeItem('buildings');
    loadBuildingsTable();
    loadBuildingLists();
    playSound('success');
    alert('تم مسح البيانات');
}
function clearWorkersData() {
    playSound('warning');
    alert('تحذير: سيتم مسح جميع بيانات العمال!');
    alert('تحذير ثاني: هذا الإجراء لا يمكن التراجع عنه!');
    if (!confirm('هل أنت متأكد؟')) return;
    const password = prompt('أدخل كلمة مرور المسح');
    if (password !== localStorage.getItem('clearPassword')) {
        playSound('error');
        return alert('كلمة مرور خاطئة');
    }
    localStorage.removeItem('workers');
    loadWorkersTable();
    playSound('success');
    alert('تم مسح البيانات');
}
function clearPartnershipData() {
    playSound('warning');
    alert('تحذير: سيتم مسح جميع بيانات الشراكة!');
    alert('تحذير ثاني: هذا الإجراء لا يمكن التراجع عنه!');
    if (!confirm('هل أنت متأكد؟')) return;
    const password = prompt('أدخل كلمة مرور المسح');
    if (password !== localStorage.getItem('clearPassword')) {
        playSound('error');
        return alert('كلمة مرور خاطئة');
    }
    localStorage.removeItem('partnerships');
    loadPartnershipTable();
    playSound('success');
    alert('تم مسح البيانات');
}
function exportAllData() {
    playSound('click');
    const data = {
        buildings: loadFromStorage('buildings'),
        workers: loadFromStorage('workers'),
        partnerships: loadFromStorage('partnerships'),
        logo: localStorage.getItem('logo'),
        exportDate: new Date().toISOString(),
        version: '2.0'
    };
    
    // إنشاء اسم الملف مع التاريخ
    const exportDate = new Date().toLocaleDateString('ar-SA', { 
        year: 'numeric', 
        month: '2-digit', 
        day: '2-digit' 
    }).replace(/\//g, '-');
    
    const filename = `نسخة_احتياطية_${exportDate}.json`;
    
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    
    // حفظ مرجع الملف في نظام المجلدات
    const fullPath = fileManager.getFilePath('backups', filename);
    fileManager.saveFileReference('backups', filename, {
        type: 'JSON_BACKUP',
        size: blob.size,
        recordsCount: {
            buildings: data.buildings?.length || 0,
            workers: data.workers?.length || 0,
            partnerships: data.partnerships?.length || 0
        }
    });
    
    // التحقق من دعم Web Share API للأجهزة المحمولة
    if (navigator.share && /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        // إنشاء ملف مؤقت للمشاركة
        const file = new File([blob], filename, { type: 'application/json' });
        navigator.share({
            files: [file],
            title: 'تصدير البيانات المحاسبية',
            text: `ملف البيانات المحاسبية - ${fullPath}`
        }).then(() => {
            playSound('success');
            alert(`تم حفظ النسخة الاحتياطية في:\n${fullPath}`);
        }).catch((error) => {
            console.log('خطأ في المشاركة، سيتم استخدام الطريقة التقليدية:', error);
            downloadFileTraditional(blob, filename, fullPath);
        });
    } else {
        // الطريقة التقليدية للتحميل
        downloadFileTraditional(blob, filename, fullPath);
    }
}

// دالة التحميل التقليدية المحسنة لـ Android
function downloadFileTraditional(blob, filename = '', fullPath = '') {
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.style.display = 'none';
    a.href = url;
    
    // تحديد اسم الملف
    if (!filename) {
        filename = 'نسخة_احتياطية_' + new Date().toISOString().slice(0,10) + '.json';
    }
    a.download = filename;
    
    // إضافة العنصر إلى DOM مؤقتاً
    document.body.appendChild(a);
    
    // محاولة التحميل بطرق متعددة
    try {
        a.click();
    } catch (e) {
        // في حالة فشل click(), استخدم dispatchEvent
        const clickEvent = new MouseEvent('click', {
            view: window,
            bubbles: true,
            cancelable: true
        });
        a.dispatchEvent(clickEvent);
    }
    
    // تنظيف
    setTimeout(() => {
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    }, 100);
    
    playSound('success');
    
    // عرض رسالة تأكيد
    setTimeout(() => {
        alert('تم إنشاء ملف التصدير. تحقق من مجلد التحميلات في جهازك.');
    }, 500);
}
document.getElementById('importAll').addEventListener('change', (e) => {
    playSound('click');
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => {
        try {
            const data = JSON.parse(ev.target.result);
            if (data.buildings) saveToStorage('buildings', data.buildings);
            if (data.workers) saveToStorage('workers', data.workers);
            if (data.partnerships) saveToStorage('partnerships', data.partnerships);
            if (data.logo) {
                localStorage.setItem('logo', data.logo);
                document.getElementById('logo').src = data.logo;
                document.getElementById('logo').style.display = 'block';
            }
            loadData();
            playSound('success');
            alert('تم الاستيراد بنجاح');
        } catch (err) {
            playSound('error');
            alert('خطأ في الاستيراد: ' + err.message);
        }
    };
    reader.readAsText(file);
});
// تعديل دالة sanitizeFilename للتعامل مع الحالات الفارغة
function sanitizeFilename(str) {
    if (!str || typeof str !== 'string') {
        return 'تقرير_غير_معروف';
    }
    return str.replace(/[/\\:*?"<>|]/g, '_').replace(/\s+/g, '_');
}
// تم تعديل هذه الدالة لتشمل معامل filename
// تعديل دالة saveAsPDF مع نظام المجلدات
function saveAsPDF(reportDivId, titleText, filename, headers, dataExtractor, totals = [], folderType = 'backups') {
    playSound('click');
    const reportDiv = document.getElementById(reportDivId);
    if (!reportDiv.innerHTML) {
        playSound('error');
        alert('يرجى إنشاء التقرير أولاً');
        return;
    }
    const doc = new jsPDF({
        orientation: 'portrait',
        unit: 'mm',
        format: 'a4'
    });
    const pageHeight = doc.internal.pageSize.height;
    const logoSrc = localStorage.getItem('logo');
    if (arabicFontBase64 !== 'YOUR_ARABIC_FONT_BASE64') {
        doc.setFont('Amiri', 'normal');
    } else {
        doc.setFont('Helvetica', 'normal');
    }
    // حساب الإجماليات للرسم في الأعلى
    const drawTotals = (doc, totals, startY) => {
        let currentY = startY;
        totals.forEach(({ label, value }) => {
            doc.setFontSize(11);
            doc.setTextColor(0, 0, 0);
            const formattedValue = formatNumberWithSeparator(value);
            const text = `${label}: ${formattedValue} ريال (${numberToArabicWords(value)})`;
            doc.text(text, 105, currentY, { align: 'center' });
            currentY += 7;
        });
        return currentY;
    };

    doc.autoTable({
        head: [headers],
        body: dataExtractor(reportDiv.querySelector('table')),
        startY: totals.length > 0 ? 65 + (totals.length * 7) : 60,
        margin: { top: totals.length > 0 ? 70 + (totals.length * 7) : 60, bottom: 20, right: 10, left: 10 },
        theme: 'grid',
        styles: {
            font: arabicFontBase64 !== 'YOUR_ARABIC_FONT_BASE64' ? 'Amiri' : 'Helvetica',
            halign: 'center',
            fontSize: 10,
            cellPadding: 2,
            overflow: 'linebreak',
            textColor: [0, 0, 0],
            lineColor: [0, 0, 0],
            lineWidth: 0.1
        },
        headStyles: { fillColor: [2, 136, 209], textColor: [255, 255, 255] },
        columnStyles: headers.reduce((acc, _, index) => ({ ...acc, [index]: { cellWidth: 'auto' } }), {}),
        didDrawPage: function (data) {
            doc.setFontSize(16);
            const titleLines = titleText.split('\n');
            let y = 15;
            titleLines.forEach(line => {
                doc.text(line, 105, y, { align: 'center' });
                y += 7;
            });
            if (logoSrc) {
                doc.addImage(logoSrc, 'PNG', 10, 10, 30, 30);
            }
            // رسم الإجماليات في أعلى الصفحة الأولى فقط
            if (data.pageNumber === 1 && totals.length > 0) {
                doc.setFontSize(12);
                doc.setTextColor(2, 136, 209);
                doc.text('ملخص الإجماليات', 105, 55, { align: 'center' });
                drawTotals(doc, totals, 62);
            }
            const pageCount = doc.internal.getNumberOfPages();
            const pageNumber = data.pageNumber;
            doc.setFontSize(10);
            doc.text(`صفحة ${pageNumber} من ${pageCount}`, 105, pageHeight - 10, { align: 'center' });
        },
        pageBreak: 'auto',
        rowHeight: 10
    });
    let finalY = doc.lastAutoTable.finalY + 10;
    totals.forEach(({ label, value }, index) => {
        const formattedValue = formatNumberWithSeparator(value);
        const textLines = doc.splitTextToSize(`${label}: ${formattedValue} ريال\n${numberToArabicWords(value)}`, 180);
        doc.setFontSize(12);
        
        // تحديد اللون حسب نوع البيان
        let textColor = [0, 0, 0]; // أسود افتراضي
        if (label.includes('نصيب يحيى')) {
            if (label.includes('يقبض')) {
                textColor = [0, 150, 0]; // أخضر للقبض
            } else {
                textColor = [200, 0, 0]; // أحمر للدفع
            }
        } else if (label.includes('نصيب صدام')) {
            if (label.includes('يقبض')) {
                textColor = [0, 100, 200]; // أزرق للقبض
            } else {
                textColor = [150, 50, 0]; // برتقالي للدفع
            }
        } else if (label.includes('الكل')) {
            textColor = [100, 0, 150]; // بنفسجي للمجموع الكلي
        }
        
        doc.setTextColor(textColor[0], textColor[1], textColor[2]);
        
        textLines.forEach(line => {
            if (finalY > pageHeight - 20) {
                doc.addPage();
                finalY = 60;
                doc.setFontSize(16);
                doc.setTextColor(0, 0, 0); // عودة للأسود للعنوان
                const titleLines = titleText.split('\n');
                let y = 15;
                titleLines.forEach(tline => {
                    doc.text(tline, 105, y, { align: 'center' });
                    y += 7;
                });
                if (logoSrc) {
                    doc.addImage(logoSrc, 'PNG', 10, 10, 30, 30);
                }
                doc.setFontSize(12);
                doc.setTextColor(textColor[0], textColor[1], textColor[2]); // عودة لللون المحدد
            }
            doc.text(line, 105, finalY, { align: 'center' });
            finalY += 10;
        });
        
        // إعادة تعيين اللون للأسود بعد كل عنصر
        doc.setTextColor(0, 0, 0);
    });
    
    // حفظ مرجع الملف في نظام المجلدات
    const fullPath = fileManager.getFilePath(folderType, filename);
    fileManager.saveFileReference(folderType, filename, {
        type: 'PDF',
        title: titleText,
        size: doc.output('blob').size
    });
    
    // استخدام طريقة حفظ محسنة لـ Android وتطبيقات الويب
    const pdfOutput = doc.output('blob');
    
    // إظهار رسالة مع المسار الكامل
    console.log(`تم حفظ الملف في: ${fullPath}`);
    
    // التحقق من بيئة التشغيل ودعم Web Share API
    if (navigator.share && /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        try {
            const file = new File([pdfOutput], filename, { type: 'application/pdf' });
            navigator.share({
                files: [file],
                title: 'تقرير PDF',
                text: `تقرير محاسبي PDF - ${fullPath}`
            }).then(() => {
                playSound('success');
                alert(`تم حفظ التقرير في:\n${fullPath}`);
            }).catch((error) => {
                console.log('خطأ في المشاركة، سيتم استخدام الطريقة التقليدية:', error);
                downloadPDFTraditional(pdfOutput, filename, fullPath);
            });
        } catch (error) {
            downloadPDFTraditional(pdfOutput, filename, fullPath);
        }
    } else {
        downloadPDFTraditional(pdfOutput, filename, fullPath);
    }
}

// دالة مساعدة لتحميل PDF بالطريقة التقليدية
function downloadPDFTraditional(pdfBlob, filename, fullPath = '') {
    const url = URL.createObjectURL(pdfBlob);
    const a = document.createElement('a');
    a.style.display = 'none';
    a.href = url;
    a.download = filename;
    
    document.body.appendChild(a);
    
    try {
        a.click();
    } catch (e) {
        // في حالة فشل click(), استخدم dispatchEvent
        const clickEvent = new MouseEvent('click', {
            view: window,
            bubbles: true,
            cancelable: true
        });
        a.dispatchEvent(clickEvent);
    }
    
    setTimeout(() => {
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    }, 100);
    
    playSound('success');
    
    // عرض رسالة تأكيد
    setTimeout(() => {
        if (fullPath) {
            alert(`تم حفظ التقرير في:\n${fullPath}\n\nتحقق من مجلد التحميلات في جهازك.`);
        } else {
            alert('تم حفظ التقرير. تحقق من مجلد التحميلات في جهازك.');
        }
    }, 500);
}
// تعديل دالة saveBuildingReportAsPDF مع إضافة تحقق
function saveBuildingReportAsPDF() {
    const building = document.getElementById('buildingReportSelect').value;
    if (!building) {
        playSound('error');
        alert('يرجى اختيار عمارة أولاً');
        return;
    }
    const buildings = loadFromStorage('buildings');
    const buildingData = buildings.find(b => b.name === building);
    if (!buildingData) {
        playSound('error');
        alert('لم يتم العثور على بيانات العمارة');
        return;
    }
    const total = buildingData.deliveries.reduce((sum, d) => sum + d.amount, 0);
    const titleText = `تقرير العماير - ${building}\nالمكان: ${buildingData.location}`;
    // استخدام اسم العمارة مباشرة مع تنظيفه
    const filename = `تقرير_العمارة_${sanitizeFilename(building)}.pdf`; // تم التحقق: الاسم صحيح
    const headers = ['المبلغ المسلم', 'المستلم', 'التاريخ (اليوم)', 'ملاحظة'];
    const dataExtractor = (table) => {
        const rows = [];
        table.querySelectorAll('tbody tr').forEach(row => {
            const cells = row.querySelectorAll('td');
            rows.push([cells[0].textContent, cells[1].textContent, cells[2].textContent, cells[3].textContent]);
        });
        return rows;
    };
    saveAsPDF('buildingReport', titleText, filename, headers, dataExtractor, [
        { label: 'الإجمالي', value: total }
    ], 'buildings');
}
// تعديل دالة saveWorkerReportAsPDF مع إضافة تحقق
function saveWorkerReportAsPDF() {
    const worker = document.getElementById('workerReportSelect').value;
    if (!worker) {
        playSound('error');
        alert('يرجى اختيار عامل أولاً');
        return;
    }
    const workers = loadFromStorage('workers');
    const workerData = workers.find(w => w.name === worker);
    if (!workerData) {
        playSound('error');
        alert('لم يتم العثور على بيانات العامل');
        return;
    }
    const startDate = document.getElementById('workerReportStart').value;
    const endDate = document.getElementById('workerReportEnd').value;
    let filteredWork = workerData.work;
    if (startDate && endDate) filteredWork = filteredWork.filter(w => w.date.split('T')[0] >= startDate && w.date.split('T')[0] <= endDate);
    const totalRemaining = filteredWork.reduce((sum, w) => sum + w.remaining, 0);
    const titleText = `تقرير العمال - ${worker}`;
    // استخدام اسم العامل مباشرة مع تنظيفه
    const filename = `تقرير_العامل_${sanitizeFilename(worker)}.pdf`; // تم التحقق: الاسم صحيح
    const headers = ['فترة العمل', 'المسلم', 'الإضافي', 'المواصلات', 'التاريخ (اليوم)', 'مكان العمل', 'اليومية', 'الباقي', 'ملاحظة'];
    const dataExtractor = (table) => {
        const rows = [];
        table.querySelectorAll('tbody tr').forEach(row => {
            const cells = row.querySelectorAll('td');
            rows.push([cells[0].textContent, cells[1].textContent, cells[2].textContent, cells[3].textContent, cells[4].textContent, cells[5].textContent, cells[6].textContent, cells[7].textContent, cells[8].textContent]);
        });
        return rows;
    };
    saveAsPDF('workerReport', titleText, filename, headers, dataExtractor, [
        { label: 'الإجمالي الباقي', value: totalRemaining }
    ], 'workers');
}
// تعديل دالة savePartnershipReportAsPDF مع تحسين اسم الملف
function savePartnershipReportAsPDF() {
    const startDate = document.getElementById('partnershipReportStart').value;
    const endDate = document.getElementById('partnershipReportEnd').value;
    let partnerships = loadFromStorage('partnerships');
    if (startDate && endDate) partnerships = partnerships.filter(d => d.date.split('T')[0] >= startDate && d.date.split('T')[0] <= endDate);
    let yahyaTotalReceived = 0, yahyaTotalPaidWorkers = 0, yahyaTotalExpenses = 0, yahyaToSaddam = 0;
    let saddamTotalReceived = 0, saddamTotalPaidWorkers = 0, saddamTotalExpenses = 0, saddamToYahya = 0;
    partnerships.forEach(d => {
        yahyaTotalReceived += d.yahyaReceived;
        yahyaTotalPaidWorkers += d.yahyaPaidWorkers;
        yahyaTotalExpenses += d.yahyaExpenses;
        yahyaToSaddam += d.yahyaToSaddam;
        saddamTotalReceived += d.saddamReceived;
        saddamTotalPaidWorkers += d.saddamPaidWorkers;
        saddamTotalExpenses += d.saddamExpenses;
        saddamToYahya += d.saddamToYahya;
    });
    const yahyaRemaining = yahyaTotalReceived - yahyaTotalPaidWorkers - yahyaTotalExpenses;
    const saddamRemaining = saddamTotalReceived - saddamTotalPaidWorkers - saddamTotalExpenses;
    const total = yahyaRemaining + saddamRemaining;
    const yahyaShare = (total / 2) - yahyaRemaining - saddamToYahya + yahyaToSaddam;
    const saddamShare = (total / 2) - saddamRemaining - yahyaToSaddam + saddamToYahya;
    const titleText = 'تقرير الشراكة';
    // تحسين تنسيق التاريخ في اسم الملف
    const printDate = new Date().toLocaleDateString('ar-SA', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\//g, '-');
    const filename = `تقرير_الشراكة_${printDate}.pdf`; // تم التعديل من "حساب" إلى "تقرير" حسب طلب المستخدم
    const headers = ['التاريخ (اليوم)', 'قبض يحيى', 'من', 'دفع للعمال', 'صرفة', 'المسلم مع صدام', 'قبض صدام', 'من', 'دفع للعمال', 'صرفة', 'المسلم مع يحيى', 'ملاحظة'];
    const dataExtractor = (table) => {
        const rows = [];
        table.querySelectorAll('tbody tr').forEach(row => {
            const cells = row.querySelectorAll('td');
            rows.push([
                cells[0].textContent,
                cells[1].textContent,
                cells[2].textContent,
                cells[3].textContent,
                cells[4].textContent,
                cells[5].textContent,
                cells[6].textContent,
                cells[7].textContent,
                cells[8].textContent,
                cells[9].textContent,
                cells[10].textContent,
                cells[11].textContent
            ]);
        });
        return rows;
    };
    saveAsPDF('settlement', titleText, filename, headers, dataExtractor, [
        { label: 'الباقي مع يحيى', value: yahyaRemaining },
        { label: 'الباقي مع صدام', value: saddamRemaining },
        { label: 'الكل', value: total },
        { label: `نصيب يحيى (${yahyaShare >= 0 ? 'يقبض' : 'يدفع'})`, value: Math.abs(yahyaShare) },
        { label: `نصيب صدام (${saddamShare >= 0 ? 'يقبض' : 'يدفع'})`, value: Math.abs(saddamShare) }
    ], 'partnership');
}
function loadData() {
    loadBuildingsTable();
    loadBuildingLists();
    loadWorkersTable();
    loadPartnershipTable();
    document.getElementById('receiver').addEventListener('change', function() {
        document.getElementById('customReceiver').style.display = this.value === 'أخرى' ? 'inline' : 'none';
    });
    const logoSrc = localStorage.getItem('logo');
    if (logoSrc) {
        document.getElementById('logo').src = logoSrc;
        document.getElementById('logo').style.display = 'block';
    }
    
    // تهيئة الجداول الديناميكية
    initializeDynamicTables();
    
    // التحقق من التنبيهات
    checkNotifications();
}
document.getElementById('logoInput').addEventListener('change', (e) => {
    playSound('click');
    const file = e.target.files[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            const logoSrc = ev.target.result;
            localStorage.setItem('logo', logoSrc);
            document.getElementById('logo').src = logoSrc;
            document.getElementById('logo').style.display = 'block';
            playSound('success');
        };
        reader.readAsDataURL(file);
    }
});
// تهيئة الجداول الديناميكية
function initializeDynamicTables() {
    // تهيئة جدول العماير (بدون إضافة حقول بحث إضافية لأنها موجودة بالفعل في HTML)
    const buildingsTable = new DynamicTable('buildingsTable');
    
    // تهيئة جدول العمال (بدون إضافة حقول بحث إضافية لأنها موجودة بالفعل في HTML)
    const workersTable = new DynamicTable('workersTable');
    
    // تهيئة جدول الشراكة (بدون إضافة حقول بحث إضافية لأنها موجودة بالفعل في HTML)
    const partnershipTable = new DynamicTable('partnershipTable');
    
    console.log('تم تهيئة الجداول الديناميكية بنجاح!');
}

// عرض هيكل المجلدات
function showFolderStructure() {
    playSound('click');
    const folders = JSON.parse(localStorage.getItem('folderStructure') || '{}');
    const display = document.getElementById('folderStructureDisplay');
    
    if (Object.keys(folders).length === 0) {
        display.innerHTML = 'لا توجد مجلدات محفوظة بعد';
    } else {
        let html = '<h4>📁 هيكل المجلدات:</h4>';
        
        Object.entries(folders).forEach(([mainFolder, data]) => {
            html += `<div style="margin-bottom: 15px;">`;
            html += `<strong>📂 ${mainFolder}/</strong> (أنشئ في: ${new Date(data.created).toLocaleDateString('ar-SA')})<br>`;
            
            if (data.subFolders) {
                Object.entries(data.subFolders).forEach(([subFolder, subData]) => {
                    const fileCount = subData.files ? subData.files.length : 0;
                    html += `&nbsp;&nbsp;📁 ${subFolder}/ (${fileCount} ملف)<br>`;
                    
                    if (subData.files && subData.files.length > 0) {
                        subData.files.slice(-3).forEach(file => {
                            const fileDate = new Date(file.created).toLocaleDateString('ar-SA');
                            html += `&nbsp;&nbsp;&nbsp;&nbsp;📄 ${file.name} (${fileDate})<br>`;
                        });
                        if (subData.files.length > 3) {
                            html += `&nbsp;&nbsp;&nbsp;&nbsp;... و ${subData.files.length - 3} ملفات أخرى<br>`;
                        }
                    }
                });
            }
            html += `</div>`;
        });
        
        display.innerHTML = html;
    }
    
    display.style.display = display.style.display === 'none' ? 'block' : 'none';
}


function sanitizeFilename(filename) {
    return filename.replace(/[\\/:*?"<>|]/g, '_').trim();
}

// Add click sounds to all buttons
document.addEventListener('DOMContentLoaded', function() {
    const buttons = document.querySelectorAll('.btn, .tab-btn');
    buttons.forEach(button => {
        button.addEventListener('click', function() {
            playSound('click');
        });
    });
});


/* --- إضافات Manus الاحترافية --- */

// 1. نظام الأمان والصلاحيات
// تم تعيين مستخدم افتراضي بصلاحيات كاملة
let currentUser = { password: '123', role: 'admin', name: 'المدير' };
const USERS = {
    'admin': { password: '123', role: 'admin', name: 'المدير' },
    'accountant': { password: '456', role: 'accountant', name: 'المحاسب' }
};

function handleLogin() {
    try {
        console.log("Login attempt...");
        if (typeof playSound === 'function') playSound('click');
        
        const userSelect = document.getElementById('userSelect');
        const passwordInput = document.getElementById('loginPassword');
        
        if (!userSelect || !passwordInput) {
            console.error("Login elements not found");
            return;
        }

        const userKey = userSelect.value;
        const password = passwordInput.value;
        
        if (USERS[userKey] && USERS[userKey].password === password) {
            console.log("Login successful for:", userKey);
            currentUser = USERS[userKey];
            if (typeof playSound === 'function') playSound('success');
            
            document.getElementById('loginScreen').style.display = 'none';
            document.getElementById('mainContent').style.display = 'block';
            
            // Execute loading sequence safely
            try { loadData(); } catch(e) { console.error("Error in loadData:", e); }
            try { applyPermissions(); } catch(e) { console.error("Error in applyPermissions:", e); }
            try { openTab('dashboard'); } catch(e) { console.error("Error in openTab:", e); }
            try { initCharts(); } catch(e) { console.error("Error in initCharts:", e); }
            try { updateHeader(); } catch(e) { console.error("Error in updateHeader:", e); }
            
        } else {
            if (typeof playSound === 'function') playSound('error');
            alert('كلمة المرور غير صحيحة!');
        }
    } catch (err) {
        console.error("Critical Login Error:", err);
        alert("حدث خطأ تقني أثناء تسجيل الدخول. يرجى المحاولة مرة أخرى.");
    }
}

function applyPermissions() {
    // تم تعديل الوظيفة للسماح بجميع الصلاحيات دائماً
    const adminOnlyElements = document.querySelectorAll('.btn-delete, .settings-tab, .admin-only');
    adminOnlyElements.forEach(el => {
        el.style.display = 'flex';
    });
}

function logout() {
    // تم تعطيل وظيفة تسجيل الخروج
    console.log("Logout disabled");
}

// 2. نظام المجلدات والنسخ الاحتياطي التلقائي
function getExportPath(type, fileName) {
    const folders = {
        'building': 'العمل/العماير/',
        'worker': 'العمل/العمال/',
        'partnership': 'العمل/الشراكة/',
        'backup': 'العمل/النسخة الاحتياطية/'
    };
    return (folders[type] || 'العمل/') + fileName;
}

async function backupData(isAuto = false) {
    try {
        const data = {
            buildings: loadFromStorage('buildings'),
            workers: loadFromStorage('workers'),
            partnerships: loadFromStorage('partnerships'),
            logo: localStorage.getItem('logo'),
            clearPassword: localStorage.getItem('clearPassword'),
            backupDate: new Date().toISOString()
        };
        
        const now = new Date();
        const dateStr = now.toLocaleDateString('ar-SA').replace(/\//g, '-');
        const timeStr = now.getHours() + '-' + now.getMinutes();
        const fileName = isAuto ? `نسخة_تلقائية_${timeStr}.json` : `نسخة_يدوية_${dateStr}_${timeStr}.json`;
        const content = JSON.stringify(data, null, 2);

        // إذا تم اختيار مجلد عمل (خاصية متقدمة لمتصفحات الكمبيوتر)
        if (typeof directoryHandle !== 'undefined' && directoryHandle) {
            try {
                const saved = await saveFileToFolder('النسخة الاحتياطية', fileName, content, false);
                if (saved && !isAuto) {
                    try { playSound('success'); } catch(e) {}
                    alert(`تم حفظ النسخة الاحتياطية بنجاح في: ${directoryHandle.name}/النسخة الاحتياطية/${fileName}`);
                    return;
                }
            } catch (e) {
                console.error("Folder save failed, falling back to download", e);
            }
        }
        
        if (isAuto) {
            localStorage.setItem('auto_backup_data', content);
            return;
        }

        // الطريقة التقليدية (المضمونة) للتحميل
        const blob = new Blob([content], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.style.display = 'none';
        a.href = url;
        a.download = fileName; 
        document.body.appendChild(a);
        
        try { playSound('click'); } catch(e) {}
        a.click();
        
        setTimeout(() => {
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
        }, 100);
        
        try { playSound('success'); } catch(e) {}
        alert('تم إنشاء النسخة الاحتياطية بنجاح. يرجى التحقق من مجلد التنزيلات (Downloads).');
        
    } catch (error) {
        console.error('Backup Error:', error);
        if (!isAuto) alert('حدث خطأ أثناء إنشاء النسخة الاحتياطية.');
    }
}

function triggerAutoBackup() {
    backupData(true);
}

// 3. الرسوم البيانية (Charts)
let charts = {};
function initCharts() {
    try {
        const buildings = loadFromStorage('buildings') || [];
        const workers = loadFromStorage('workers') || [];
        const partnerships = loadFromStorage('partnerships') || [];

        // حساب الإحصائيات السريعة
        const totalBuildings = buildings.reduce((sum, b) => sum + (b.deliveries || []).reduce((s, d) => s + (Number(d.amount) || 0), 0), 0);
        const totalWorkers = workers.reduce((sum, w) => sum + (w.work || []).reduce((s, wk) => s + (Number(wk.remaining) || 0), 0), 0);
        
        const totalYahyaRemaining = partnerships.reduce((sum, p) => sum + (Number(p.yahyaReceived) || 0) - (Number(p.yahyaPaidWorkers) || 0) - (Number(p.yahyaExpenses) || 0), 0);
        const totalSaddamRemaining = partnerships.reduce((sum, p) => sum + (Number(p.saddamReceived) || 0) - (Number(p.saddamPaidWorkers) || 0) - (Number(p.saddamExpenses) || 0), 0);
        const partnershipBalance = totalYahyaRemaining + totalSaddamRemaining;

        // تحديث البطاقات
        document.getElementById('stat-total-buildings').textContent = formatNumberWithSeparator(totalBuildings);
        document.getElementById('stat-total-workers').textContent = formatNumberWithSeparator(totalWorkers);
        document.getElementById('stat-partnership-balance').textContent = formatNumberWithSeparator(partnershipBalance);

        // 1. رسوم العماير
        const activeBuildings = buildings.filter(b => !b.isPaid);
        const buildingLabels = activeBuildings.length > 0 ? activeBuildings.map(b => b.name) : ['لا توجد عماير نشطة'];
        const buildingData = activeBuildings.length > 0 ? activeBuildings.map(b => (b.deliveries || []).reduce((sum, d) => sum + (Number(d.amount) || 0), 0)) : [0];
        renderChart('buildingsChart', 'bar', buildingLabels, buildingData, 'المسلمات (ريال)', '#0288d1');

        // 2. رسوم العمال
        const activeWorkers = workers.filter(w => (w.work || []).reduce((s, wk) => s + (Number(wk.remaining) || 0), 0) > 0);
        const workerLabels = activeWorkers.length > 0 ? activeWorkers.map(w => w.name) : ['لا توجد مستحقات'];
        const workerData = activeWorkers.length > 0 ? activeWorkers.map(w => (w.work || []).reduce((sum, wk) => sum + (Number(wk.remaining) || 0), 0)) : [0];
        renderChart('workersChart', 'doughnut', workerLabels, workerData, 'المستحقات المتبقية', ['#43a047', '#e53935', '#ffb300', '#7b1fa2', '#ff5722', '#2196f3']);

        // 3. رسوم الشراكة (آخر 10 عمليات)
        const lastPartnerships = partnerships.slice(-10);
        if (lastPartnerships.length > 0) {
            renderPartnershipChart(
                lastPartnerships.map(p => {
                    const d = new Date(p.date);
                    const day = String(d.getDate()).padStart(2, '0');
                    const month = String(d.getMonth() + 1).padStart(2, '0');
                    const year = d.getFullYear();
                    const hours = d.getHours();
                    const minutes = String(d.getMinutes()).padStart(2, '0');
                    const ampm = hours >= 12 ? 'م' : 'ص';
                    const displayHours = hours % 12 || 12;
                    return `${displayHours}:${minutes}${ampm} ${day}/${month}/${year}`;
                }), 
                lastPartnerships.map(p => Number(p.yahyaReceived) || 0), 
                lastPartnerships.map(p => Number(p.saddamReceived) || 0)
            );
        } else {
            renderPartnershipChart(['لا توجد بيانات'], [0], [0]);
        }
    } catch (error) {
        console.error("Error in initCharts:", error);
    }
}

function renderChart(canvasId, type, labels, data, label, colors) {
    const canvas = document.getElementById(canvasId);
    if (!canvas) return;
    if (charts[canvasId]) charts[canvasId].destroy();
    charts[canvasId] = new Chart(canvas.getContext('2d'), {
        type: type,
        data: { labels: labels, datasets: [{ label: label, data: data, backgroundColor: colors, borderWidth: 1 }] },
        options: { responsive: true, plugins: { legend: { display: type === 'pie' } } }
    });
}

function renderPartnershipChart(labels, yahya, saddam) {
    const canvas = document.getElementById('partnershipChart');
    if (!canvas) return;
    if (charts['partnershipChart']) charts['partnershipChart'].destroy();
    charts['partnershipChart'] = new Chart(canvas.getContext('2d'), {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'قبض يحيى', data: yahya, borderColor: '#0288d1', fill: false }, { label: 'قبض صدام', data: saddam, borderColor: '#ff5722', fill: false }] },
        options: { responsive: true }
    });
}

// 4. الوضع الليلي (Dark Mode)
function toggleDarkMode() {
    const body = document.body;
    const newTheme = body.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
    body.setAttribute('data-theme', newTheme);
    localStorage.setItem('theme', newTheme);
    document.getElementById('darkModeToggle').textContent = newTheme === 'dark' ? '☀️' : '🌙';
    playSound('click');
}

// 5. تصدير Excel المنظم
function exportToExcel(tableId, filename, type = 'building') {
    playSound('click');
    const table = document.getElementById(tableId);
    if (!table) return;
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.table_to_sheet(table);
    XLSX.utils.book_append_sheet(wb, ws, "التقرير");
    const fullFileName = `${filename}_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.xlsx`;
    XLSX.writeFile(wb, getExportPath(type, fullFileName));
    playSound('success');
}

function exportBuildingReportToExcel() {
    const building = document.getElementById('buildingReportSelect').value;
    if (!building) return alert('يرجى اختيار عمارة أولاً');
    exportToExcel('buildingReportTable', `تقرير_عمارة_${building}`, 'building');
}

function exportWorkerReportToExcel() {
    const workerSelect = document.getElementById('workerReportSelect');
    if (!workerSelect) return;
    
    const worker = workerSelect.value;
    if (!worker) return alert('يرجى اختيار عامل أولاً');
    
    const table = document.getElementById('workerReportTable');
    if (!table) {
        // إذا لم يكن الجدول موجوداً، ربما نحتاج لإنشاء التقرير أولاً
        generateWorkerReport();
    }
    
    // إعادة التحقق بعد محاولة التوليد
    if (document.getElementById('workerReportTable')) {
        exportToExcel('workerReportTable', `تقرير_عامل_${worker}`, 'worker');
    } else {
        alert('لا توجد بيانات لتصديرها');
    }
}

function exportPartnershipReportToExcel() {
    exportToExcel('partnershipReportTable', 'تقرير_الشراكة', 'partnership');
}

// 6. التنبيهات
function checkNotifications(currentTab = 'dashboard') {
    const workers = loadFromStorage('workers');
    const buildings = loadFromStorage('buildings');
    const partnerships = loadFromStorage('partnerships');
    const area = document.getElementById('notificationArea');
    if (!area) return;
    area.innerHTML = '';
    
    // 1. تنبيهات العمال - تظهر فقط في قسم العمال
    if (currentTab === 'workers') {
        workers.forEach(w => {
            const totalRemaining = w.work.reduce((sum, wk) => sum + wk.remaining, 0);
            if (totalRemaining > 50000) {
                const div = document.createElement('div');
                div.className = 'notification notification-danger';
                div.innerHTML = `⚠️ تنبيه: مستحقات العامل <strong>${w.name}</strong> تجاوزت 50,000 ريال (المبلغ: ${formatNumberWithSeparator(totalRemaining)} ريال)`;
                area.appendChild(div);
            } else if (totalRemaining > 10000) {
                const div = document.createElement('div');
                div.className = 'notification notification-warning';
                div.innerHTML = `⚠️ تنبيه: مستحقات العامل <strong>${w.name}</strong> بلغت ${formatNumberWithSeparator(totalRemaining)} ريال`;
                area.appendChild(div);
            }
        });
    }

    // 2. تنبيهات العماير - تظهر فقط في قسم العماير
    if (currentTab === 'buildings') {
        const oneMonthAgo = new Date();
        oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);

        buildings.forEach(b => {
            if (!b.isPaid) {
                let lastDeliveryDate = null;
                if (b.deliveries && b.deliveries.length > 0) {
                    const dates = b.deliveries.map(d => new Date(d.date));
                    lastDeliveryDate = new Date(Math.max.apply(null, dates));
                } else {
                    lastDeliveryDate = new Date(b.id);
                }

                if (lastDeliveryDate < oneMonthAgo) {
                    const diffDays = Math.floor((new Date() - lastDeliveryDate) / (1000 * 60 * 60 * 24));
                    const div = document.createElement('div');
                    div.className = 'notification notification-info';
                    div.innerHTML = `🏢 تنبيه: عمارة <strong>${b.name}</strong> لم يتم التسديد لها منذ ${diffDays} يوم (آخر دفعة: ${lastDeliveryDate.toLocaleDateString('ar-EG')})`;
                    area.appendChild(div);
                }
            }
        });
    }

    // 3. تنبيهات الشراكة - تظهر فقط في قسم الشراكة
    if (currentTab === 'partnership') {
        let yahyaTotalReceived = 0, yahyaTotalPaidWorkers = 0, yahyaTotalExpenses = 0, yahyaToSaddam = 0;
        let saddamTotalReceived = 0, saddamTotalPaidWorkers = 0, saddamTotalExpenses = 0, saddamToYahya = 0;
        
        partnerships.forEach(d => {
            yahyaTotalReceived += d.yahyaReceived;
            yahyaTotalPaidWorkers += d.yahyaPaidWorkers;
            yahyaTotalExpenses += d.yahyaExpenses;
            yahyaToSaddam += d.yahyaToSaddam;
            saddamTotalReceived += d.saddamReceived;
            saddamTotalPaidWorkers += d.saddamPaidWorkers;
            saddamTotalExpenses += d.saddamExpenses;
            saddamToYahya += d.saddamToYahya;
        });
        
        const yahyaRemaining = yahyaTotalReceived - yahyaTotalPaidWorkers - yahyaTotalExpenses;
        const saddamRemaining = saddamTotalReceived - saddamTotalPaidWorkers - saddamTotalExpenses;
        const total = yahyaRemaining + saddamRemaining;
        const yahyaShare = (total / 2) - yahyaRemaining - saddamToYahya + yahyaToSaddam;
        const saddamShare = (total / 2) - saddamRemaining - yahyaToSaddam + saddamToYahya;

        if (Math.abs(yahyaShare) > 500000) {
            const div = document.createElement('div');
            div.className = 'notification notification-danger';
            div.innerHTML = `🤝 تنبيه شراكة: نصيب يحيى (${formatNumberWithSeparator(yahyaShare)} ريال) تجاوز 500,000 ريال`;
            area.appendChild(div);
        }
        if (Math.abs(saddamShare) > 500000) {
            const div = document.createElement('div');
            div.className = 'notification notification-danger';
            div.innerHTML = `🤝 تنبيه شراكة: نصيب صدام (${formatNumberWithSeparator(saddamShare)} ريال) تجاوز 500,000 ريال`;
            area.appendChild(div);
        }
    }
}

// 7. ربط الدوال بالواجهة
const originalOpenTab = openTab;openTab = function(tabName) {
    if (currentUser && currentUser.role !== 'admin' && tabName === 'settings') {
        playSound('error');
        alert('عذراً، لا تملك صلاحية الدخول للإعدادات');
        return;
    }
    originalOpenTab(tabName);
    if (tabName === 'dashboard') {
        initCharts();
    }
    checkNotifications(tabName);
    document.querySelectorAll('.nav-item').forEach(item => {
        item.classList.remove('active');
        if (item.getAttribute('onclick') && item.getAttribute('onclick').includes(`'${tabName}'`)) item.classList.add('active');
    });
};

const originalSaveToStorage = saveToStorage;
saveToStorage = function(key, data) {
    originalSaveToStorage(key, data);
    triggerAutoBackup();
};

const originalUpdateHeader = updateHeader;
updateHeader = function() {
    const now = new Date();
    const dayName = getDayName(now.toISOString().split('T')[0]);
    const date = now.toLocaleDateString('ar-EG');
    const userDisplay = currentUser ? `<div style="font-size: 0.8rem; color: var(--primary-color);">👤 ${currentUser.name}</div>` : '';
    const info = document.getElementById('headerInfo');
    if (info) info.innerHTML = `<h1 style="margin:0; font-size: 1.1rem;">النظام المحاسبي</h1><div style="font-size: 0.7rem; opacity: 0.7;">${dayName} - ${date}</div>${userDisplay}`;
};

document.addEventListener('DOMContentLoaded', () => {
    const savedTheme = localStorage.getItem('theme') || 'light';
    document.body.setAttribute('data-theme', savedTheme);
    const toggle = document.getElementById('darkModeToggle');
    if (toggle) toggle.textContent = savedTheme === 'dark' ? '☀️' : '🌙';
    
    // تشغيل البرنامج تلقائياً
    console.log("Auto-starting program...");
    const isPasswordEnabled = localStorage.getItem('passwordProtectionEnabled') === 'true';
    if (isPasswordEnabled) {
        document.getElementById('passwordScreen').style.display = 'block';
        document.getElementById('mainContent').style.display = 'none';
    } else {
        document.getElementById('passwordScreen').style.display = 'none';
        document.getElementById('mainContent').style.display = 'block';
        initApp();
    }

    // تحميل إعدادات النسخ الاحتياطي التلقائي وفحص الجدولة
    loadAutoBackupSettings();
    checkAndPerformAutoBackup();
});

/* --- نظام إدارة المجلدات المتقدم (File System Access API) --- */

let directoryHandle = null;

// دالة اختيار المجلد الرئيسي
async function selectWorkFolder() {
    try {
        directoryHandle = await window.showDirectoryPicker({
            mode: 'readwrite'
        });
        
        // التحقق من الصلاحيات
        const permission = await directoryHandle.requestPermission({ mode: 'readwrite' });
        if (permission !== 'granted') {
            alert('يجب الموافقة على صلاحيات الوصول لحفظ الملفات تلقائياً');
            return;
        }

        document.getElementById('folderStatus').textContent = `✅ المجلد المختار: ${directoryHandle.name}`;
        document.getElementById('folderStatus').style.color = 'var(--success-color)';
        playSound('success');
        
        // إنشاء الهيكل الداخلي تلقائياً
        await createFolderStructure();
        
    } catch (err) {
        console.error('Error selecting folder:', err);
        if (err.name !== 'AbortError') {
            alert('هذا المتصفح قد لا يدعم ميزة اختيار المجلدات، سيتم استخدام التنزيلات الافتراضية.');
        }
    }
}

// إنشاء هيكل المجلدات (العماير، العمال، الشراكة، النسخة الاحتياطية)
async function createFolderStructure() {
    if (!directoryHandle) return;
    const folders = ['العماير', 'العمال', 'الشراكة', 'النسخة الاحتياطية'];
    for (const folder of folders) {
        await directoryHandle.getDirectoryHandle(folder, { create: true });
    }
}

// دالة حفظ ملف مباشرة في المجلد المختار
async function saveFileToFolder(folderName, fileName, content, isBlob = true) {
    if (!directoryHandle) return false;

    try {
        const subDir = await directoryHandle.getDirectoryHandle(folderName, { create: true });
        const fileHandle = await subDir.getFileHandle(fileName, { create: true });
        const writable = await fileHandle.createWritable();
        
        if (isBlob) {
            await writable.write(content);
        } else {
            await writable.write(new Blob([content], { type: 'text/plain' }));
        }
        
        await writable.close();
        return true;
    } catch (err) {
        console.error('Error saving file to folder:', err);
        return false;
    }
}

// تحديث دالة النسخ الاحتياطي لتدعم الحفظ المباشر
const originalBackupData = backupData;
backupData = async function(isAuto = false) {
    const data = {
        buildings: loadFromStorage('buildings'),
        workers: loadFromStorage('workers'),
        partnerships: loadFromStorage('partnerships'),
        logo: localStorage.getItem('logo'),
        clearPassword: localStorage.getItem('clearPassword'),
        backupDate: new Date().toISOString()
    };
    
    const fileName = isAuto ? `نسخة_تلقائية_${new Date().getHours()}-${new Date().getMinutes()}.json` : `نسخة_يدوية_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.json`;
    const content = JSON.stringify(data, null, 2);

    if (directoryHandle) {
        const saved = await saveFileToFolder('النسخة الاحتياطية', fileName, content, false);
        if (saved && !isAuto) {
            playSound('success');
            alert(`تم حفظ النسخة الاحتياطية بنجاح في: ${directoryHandle.name}/النسخة الاحتياطية/${fileName}`);
            return;
        }
    }
    
    // العودة للطريقة التقليدية إذا لم يتم اختيار مجلد أو فشل الحفظ
    if (!isAuto) {
        const blob = new Blob([content], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `العمل/النسخة الاحتياطية/${fileName}`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        playSound('success');
    } else {
        localStorage.setItem('auto_backup_data', content);
    }
};

// تحديث دالة تصدير Excel لتدعم الحفظ المباشر
const originalExportToExcel = exportToExcel;
exportToExcel = async function(tableId, filename, type = 'building') {
    playSound('click');
    const table = document.getElementById(tableId);
    if (!table) return;
    
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.table_to_sheet(table);
    XLSX.utils.book_append_sheet(wb, ws, "التقرير");
    
    const fullFileName = `${filename}_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.xlsx`;
    
    if (directoryHandle) {
        const folderMap = { 'building': 'العماير', 'worker': 'العمال', 'partnership': 'الشراكة', 'backup': 'النسخة الاحتياطية' };
        const folderName = folderMap[type] || 'العمل';
        
        // مكتبة XLSX.write تعيد البيانات كـ ArrayBuffer
        const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
        const saved = await saveFileToFolder(folderName, fullFileName, excelBuffer, true);
        
        if (saved) {
            playSound('success');
            alert(`تم تصدير الملف بنجاح إلى: ${directoryHandle.name}/${folderName}/${fullFileName}`);
            return;
        }
    }
    
    // الطريقة التقليدية
    XLSX.writeFile(wb, getExportPath(type, fullFileName));
    playSound('success');
};


/* --- إصلاحات Manus النهائية --- */

// 1. تعريف دالة importData المفقودة لربط الزر الأول بالوظيفة المطلوبة
function importData() {
    if (typeof playSound === 'function') playSound('click');
    // الزرين يقومان بنفس العمل (استيراد ملف JSON)، لذا سنقوم بتوجيه هذا الزر لفتح نافذة اختيار الملفات
    const importInput = document.getElementById('importAll');
    if (importInput) {
        importInput.click();
    } else {
        alert('خطأ: لم يتم العثور على عنصر استيراد البيانات في الصفحة.');
    }
}

// 2. إصلاح دالة backupData لضمان عملها بشكل مضمون وتجاوز مشاكل مسارات الملفات
// نقوم بإعادة تعريفها لتجنب أي تعارضات سابقة وللتأكد من أنها تعمل في كل الظروف
var backupData = async function(isAuto = false) {
    try {
        const data = {
            buildings: typeof loadFromStorage === 'function' ? loadFromStorage('buildings') : [],
            workers: typeof loadFromStorage === 'function' ? loadFromStorage('workers') : [],
            partnerships: typeof loadFromStorage === 'function' ? loadFromStorage('partnerships') : [],
            logo: localStorage.getItem('logo'),
            clearPassword: localStorage.getItem('clearPassword'),
            backupDate: new Date().toISOString()
        };
        
        const now = new Date();
        const dateStr = now.toLocaleDateString('ar-SA').replace(/\//g, '-');
        const timeStr = now.getHours() + '-' + now.getMinutes();
        const fileName = isAuto ? `نسخة_تلقائية_${timeStr}.json` : `نسخة_يدوية_${dateStr}_${timeStr}.json`;
        const content = JSON.stringify(data, null, 2);

        // محاولة الحفظ في المجلد المختار إذا كان متاحاً (FileSystem Access API)
        if (typeof directoryHandle !== 'undefined' && directoryHandle) {
            try {
                const saved = await saveFileToFolder('النسخة الاحتياطية', fileName, content, false);
                if (saved && !isAuto) {
                    if (typeof playSound === 'function') playSound('success');
                    alert(`تم حفظ النسخة الاحتياطية بنجاح في: ${directoryHandle.name}/النسخة الاحتياطية/${fileName}`);
                    return;
                }
            } catch (e) {
                console.warn("فشل الحفظ في المجلد، سيتم الانتقال للتحميل التقليدي", e);
            }
        }
        
        // الطريقة التقليدية والمضمونة للتحميل في جميع المتصفحات
        if (!isAuto) {
            const blob = new Blob([content], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.style.display = 'none';
            a.href = url;
            // هام: لا تضع مسارات مجلدات هنا (مثل "العمل/..") لأن المتصفحات تمنع ذلك لأسباب أمنية
            a.download = fileName; 
            document.body.appendChild(a);
            a.click();
            
            setTimeout(() => {
                document.body.removeChild(a);
                URL.revokeObjectURL(url);
            }, 100);
            
            if (typeof playSound === 'function') playSound('success');
            alert('تم إنشاء النسخة الاحتياطية بنجاح. ستجد الملف في مجلد "التنزيلات" (Downloads) بجهازك.');
        } else {
            // حفظ نسخة تلقائية في ذاكرة المتصفح
            localStorage.setItem('auto_backup_data', content);
        }
    } catch (error) {
        console.error('Backup Error:', error);
        alert('حدث خطأ أثناء النسخ الاحتياطي: ' + error.message);
    }
};

// 3. تحسين واجهة المستخدم في index.html عبر إخفاء الزر المكرر (اختياري ولكن يفضل عمله عبر JS)
document.addEventListener('DOMContentLoaded', () => {
    // بما أن المستخدم سأل عن الزرين، سنبقي عليهما ولكن سنجعلهما يعملان بنفس الطريقة
    // أو يمكننا تمييز أحدهما، ولكن الأفضل هو توحيد الوظيفة لضمان عدم الارتباك
    console.log("Manus Fixes Applied: Backup and Import issues resolved.");
});

/* Modal Functions */
function showModal(title, content, printFn = null, exportFn = null) {
    document.getElementById('modalTitle').textContent = title;
    document.getElementById('modalBody').innerHTML = content;
    
    const printBtn = document.getElementById('modalPrintBtn');
    const exportBtn = document.getElementById('modalExportBtn');
    
    if (printBtn) {
        if (printFn) {
            printBtn.style.display = 'inline-flex';
            printBtn.onclick = () => { printFn(); };
        } else {
            printBtn.style.display = 'none';
        }
    }
    
    if (exportBtn) {
        exportBtn.style.display = 'none';
    }
    
    document.getElementById('universalModal').style.display = 'block';
    document.body.style.overflow = 'hidden'; // Prevent scrolling background
}

function closeModal() {
    document.getElementById('universalModal').style.display = 'none';
    document.body.style.overflow = 'auto';
}

// Close modal when clicking outside
window.onclick = function(event) {
    const modal = document.getElementById('universalModal');
    if (event.target == modal) {
        closeModal();
    }
}

function showTableInModal(tableId, title) {
    playSound('click');
    
    // تحديث البيانات في الجداول الأصلية قبل عرضها
    if (tableId === 'partnershipTable') {
        loadPartnershipTable();
    } else if (tableId === 'workersTable') {
        loadWorkersTable();
    } else if (tableId === 'workersListTable') {
        loadWorkersTable();
    } else if (tableId === 'buildingsTable') {
        loadBuildingsTable();
    }
    
    const table = document.getElementById(tableId);
    // Clone the table to show in modal without moving it from its original place
    const tableClone = table.cloneNode(true);
    tableClone.style.display = 'table';
    
    // إضافة ملخص الإجماليات للجداول
    let summaryHTML = '';
    if (tableId === 'buildingsTable') {
        const buildings = loadFromStorage('buildings');
        const totalAmount = buildings.reduce((sum, b) => sum + b.deliveries.reduce((s, d) => s + d.amount, 0), 0);
        summaryHTML = `<div style="background: rgba(2, 136, 209, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--primary-color);">
            <p style="margin: 0 0 10px 0; font-weight: bold; color: var(--primary-color);">📊 إجمالي المسلمات</p>
            <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--primary-color);">${formatNumberWithSeparator(totalAmount)}</p>
            <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.9rem;">ريال</p>
            <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.85rem;">${numberToArabicWords(totalAmount)}</p>
        </div>`;
    } else if (tableId === 'workersTable' || tableId === 'workersListTable') {
        const workers = loadFromStorage('workers') || [];
        const totalRemaining = workers.reduce((sum, w) => sum + (w.work || []).reduce((s, wk) => s + (Number(wk.remaining) || 0), 0), 0);
        summaryHTML = `<div style="background: rgba(229, 57, 53, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--danger-color);">
            <p style="margin: 0 0 10px 0; font-weight: bold; color: var(--danger-color);">💰 إجمالي الباقي</p>
            <p style="margin: 0; font-size: 1.3rem; font-weight: bold; color: var(--danger-color);">${formatNumberWithSeparator(totalRemaining)}</p>
            <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.9rem;">ريال</p>
            <p style="margin: 5px 0 0 0; color: var(--nav-text); font-size: 0.85rem;">${numberToArabicWords(totalRemaining)}</p>
        </div>`;
    } else if (tableId === 'partnershipTable') {
        const partnerships = loadFromStorage('partnerships');
        let yahyaTotalReceived = 0, yahyaTotalPaidWorkers = 0, yahyaTotalExpenses = 0;
        let saddamTotalReceived = 0, saddamTotalPaidWorkers = 0, saddamTotalExpenses = 0;
        partnerships.forEach(d => {
            yahyaTotalReceived += d.yahyaReceived;
            yahyaTotalPaidWorkers += d.yahyaPaidWorkers;
            yahyaTotalExpenses += d.yahyaExpenses;
            saddamTotalReceived += d.saddamReceived;
            saddamTotalPaidWorkers += d.saddamPaidWorkers;
            saddamTotalExpenses += d.saddamExpenses;
        });
        const yahyaRemaining = yahyaTotalReceived - yahyaTotalPaidWorkers - yahyaTotalExpenses;
        const saddamRemaining = saddamTotalReceived - saddamTotalPaidWorkers - saddamTotalExpenses;
        const total = yahyaRemaining + saddamRemaining;
        summaryHTML = `<div style="background: rgba(67, 160, 71, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px; border-right: 4px solid var(--success-color);">
            <p style="margin: 0 0 15px 0; font-weight: bold; color: var(--success-color);">📊 ملخص الأرصدة</p>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div>
                    <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الباقي مع يحيى</p>
                    <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(yahyaRemaining)}</p>
                </div>
                <div>
                    <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الباقي مع صدام</p>
                    <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(saddamRemaining)}</p>
                </div>
                <div style="grid-column: 1 / -1;">
                    <p style="margin: 0 0 5px 0; color: var(--nav-text); font-size: 0.9rem;">الإجمالي</p>
                    <p style="margin: 0; font-size: 1.1rem; font-weight: bold; color: var(--success-color);">${formatNumberWithSeparator(total)}</p>
                </div>
            </div>
        </div>`;
    }
    
    showModal(title, summaryHTML + '<div class="table-container">' + tableClone.outerHTML + '</div>', null, null);
    
    // إعادة ربط الأحداث للأزرار في الجدول المنسوخ
    reattachTableEvents(tableId);
}

// دالة مساعدة لإعادة ربط أحداث الأزرار في الجداول المعروضة في Modal
function reattachTableEvents(tableId) {
    const modalBody = document.getElementById('modalBody');
    if (!modalBody) return;
    
    // استخدام event delegation لتبسيط الربط وضمان العمل حتى مع التغييرات الديناميكية
    modalBody.onclick = function(e) {
        const target = e.target;
        
        // التحقق من زر التعديل
        if (target.classList.contains('btn-info') || target.closest('.btn-info')) {
            const btn = target.classList.contains('btn-info') ? target : target.closest('.btn-info');
            const onclickAttr = btn.getAttribute('onclick');
            if (onclickAttr && onclickAttr.includes('openEditModal')) {
                e.preventDefault();
                e.stopPropagation();
                // استخراج المعاملات من onclick
                const match = onclickAttr.match(/openEditModal\('([^']*)', '([^']*)', ([^)]*)\)/);
                if (match) {
                    const type = match[1];
                    const id = match[2];
                    const subId = match[3] === 'null' ? null : match[3].replace(/'/g, '');
                    closeModal();
                    setTimeout(() => {
                        openEditModal(type, id, subId);
                    }, 100);
                }
            } else if (onclickAttr && onclickAttr.includes('editPartnership')) {
                const match = onclickAttr.match(/editPartnership\((\d+)\)/);
                if (match && match[1]) {
                    e.preventDefault();
                    e.stopPropagation();
                    closeModal();
                    setTimeout(() => {
                        editPartnership(parseInt(match[1]));
                    }, 100);
                }
            }
        }
        
        // التحقق من زر الحذف
        if (target.classList.contains('btn-delete') || target.closest('.btn-delete')) {
            const btn = target.classList.contains('btn-delete') ? target : target.closest('.btn-delete');
            const onclickAttr = btn.getAttribute('onclick');
            if (onclickAttr && onclickAttr.includes('deletePartnership')) {
                const match = onclickAttr.match(/deletePartnership\((\d+)\)/);
                if (match && match[1]) {
                    e.preventDefault();
                    e.stopPropagation();
                    deletePartnership(parseInt(match[1]));
                    // لا نغلق المودال هنا للسماح للمستخدم برؤية الحذف في الجدول الأصلي إذا أراد
                }
            }
        }
        
        // التحقق من زر التصفية
        if (target.classList.contains('btn-save') && target.textContent.includes('تصفية')) {
            e.preventDefault();
            e.stopPropagation();
            closeModal();
            setTimeout(() => {
                openTab('partnership');
                settlePartnership();
            }, 100);
        }
    };
}

// Helper functions to export full tables (if not already existing)
function exportBuildingsToExcel() {
    const buildings = loadFromStorage('buildings');
    let data = [];
    buildings.forEach(b => {
        b.deliveries.forEach(d => {
            data.push({
                'العمارة': b.name,
                'المكان': b.location,
                'المبلغ': d.amount,
                'المستلم': d.receiver,
                'التاريخ': d.date,
                'ملاحظة': d.note
            });
        });
    });
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "المسلمات");
    XLSX.writeFile(wb, "جدول_المسلمات.xlsx");
}

function exportWorkersToExcel() {
    const workers = loadFromStorage('workers');
    let data = [];
    workers.forEach(w => {
        w.work.forEach(work => {
            data.push({
                'العامل': w.name,
                'التاريخ': work.date,
                'الفترة': work.period,
                'المسلم': work.delivered,
                'الإضافي': work.extra,
                'المواصلات': work.transport,
                'المكان': work.building,
                'ملاحظة': work.note
            });
        });
    });
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "عمل_العمال");
    XLSX.writeFile(wb, "جدول_العمال.xlsx");
}

// --- نظام أرشفة العماير الجديد ---
function archiveBuilding(buildingId) {
    playSound('confirm');
    if (!confirm('هل أنت متأكد من نقل هذه العمارة إلى الأرشيف؟ ستختفي من القوائم النشطة.')) return;
    
    let buildings = loadFromStorage('buildings');
    let archived = loadFromStorage('archived_buildings');
    
    const index = buildings.findIndex(b => b.id == buildingId);
    if (index !== -1) {
        const building = buildings.splice(index, 1)[0];
        building.archivedDate = new Date().toISOString();
        archived.push(building);
        
        saveToStorage('buildings', buildings);
        saveToStorage('archived_buildings', archived);
        
        closeModal();
        loadBuildingsTable();
        loadBuildingLists();
        playSound('success');
        alert('تم نقل العمارة للأرشيف بنجاح');
    }
}

function unarchiveBuilding(buildingId) {
    playSound('confirm');
    if (!confirm('هل تريد استعادة هذه العمارة من الأرشيف؟')) return;
    
    let buildings = loadFromStorage('buildings');
    let archived = loadFromStorage('archived_buildings');
    
    const index = archived.findIndex(b => b.id == buildingId);
    if (index !== -1) {
        const building = archived.splice(index, 1)[0];
        delete building.archivedDate;
        buildings.push(building);
        
        saveToStorage('buildings', buildings);
        saveToStorage('archived_buildings', archived);
        
        playSound('success');
        alert('تم استعادة العمارة بنجاح');
        showArchivedBuildings(); // تحديث عرض الأرشيف
        loadBuildingsTable();
        loadBuildingLists();
    }
}

function showArchivedBuildings() {
    const archived = loadFromStorage('archived_buildings');
    let content = '<div class="list-box"><ul>';
    if (archived.length === 0) {
        content += '<li style="text-align:center; padding:20px;">الأرشيف فارغ حالياً</li>';
    } else {
        archived.forEach(b => {
            content += `
            <li style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid var(--border-color);">
                <div>
                    <span style="font-weight:bold;">${b.name}</span><br>
                    <small style="color:var(--nav-text)">${b.location} - مؤرشف في: ${new Date(b.archivedDate).toLocaleDateString('ar-EG')}</small>
                </div>
                <div style="display:flex; gap:5px;">
                    <button class="btn btn-save" style="padding:5px 10px; font-size:0.8rem;" onclick="showArchivedBuildingDetails('${b.id}')">عرض</button>
                    <button class="btn btn-import" style="padding:5px 10px; font-size:0.8rem;" onclick="unarchiveBuilding('${b.id}')">استعادة</button>
                    <button class="btn btn-delete" style="padding:5px 10px; font-size:0.8rem;" onclick="deleteArchivedBuilding('${b.id}')">حذف</button>
                </div>
            </li>`;
        });
    }
    content += '</ul></div>';
    showModal('📦 أرشيف العماير', content);
}

function showArchivedBuildingDetails(buildingId) {
    const archived = loadFromStorage('archived_buildings');
    const b = archived.find(item => item.id == buildingId);
    
    if (!b) return alert('تعذر العثور على البيانات');

    let total = b.deliveries.reduce((sum, d) => sum + parseNumber(d.amount), 0);
    
    let content = `
        <div style="margin-bottom:20px; padding:15px; background:var(--bg-color); border-radius:10px; border:1px solid var(--border-color);">
            <p><strong>اسم العمارة:</strong> ${b.name}</p>
            <p><strong>المكان:</strong> ${b.location}</p>
            <p><strong>تاريخ الأرشفة:</strong> ${new Date(b.archivedDate).toLocaleDateString('ar-EG')}</p>
            <p><strong>إجمالي المسلمات:</strong> <span style="color:var(--primary-color); font-weight:bold;">${formatNumberWithSeparator(total)} ريال</span></p>
        </div>
        <h3>سجل المسلمات (أرشيف)</h3>
        <div class="table-container">
            <table class="dynamic-table">
                <thead>
                    <tr>
                        <th>المبلغ</th>
                        <th>المستلم</th>
                        <th>التاريخ</th>
                        <th>ملاحظة</th>
                    </tr>
                </thead>
                <tbody>
                    ${b.deliveries.length > 0 ? b.deliveries.map(d => `
                        <tr>
                            <td>${formatNumberWithSeparator(parseNumber(d.amount))}</td>
                            <td>${d.receiver}</td>
                            <td>${getDateTimeString(d.date)}</td>
                            <td>${d.note || '-'}</td>
                        </tr>
                    `).join('') : '<tr><td colspan="4" style="text-align:center;">لا توجد مسلمات مسجلة</td></tr>'}
                </tbody>
            </table>
        </div>
        <div style="margin-top:20px;">
            <button class="btn btn-save" onclick="showArchivedBuildings()">العودة للأرشيف</button>
        </div>
    `;
    
    showModal(`تفاصيل مؤرشفة: ${b.name}`, content);
}

function deleteArchivedBuilding(buildingId) {
    if (!requireDeletePassword()) return;
    if (!confirm('تحذير: سيتم حذف العمارة نهائياً من الأرشيف. هل أنت متأكد؟')) return;
    
    let archived = loadFromStorage('archived_buildings');
    archived = archived.filter(b => b.id != buildingId);
    saveToStorage('archived_buildings', archived);
    
    playSound('success');
    alert('تم الحذف النهائي بنجاح');
    showArchivedBuildings();
}

// --- ميزة البحث الذكي واللحظي ---
function filterTable(tableId, query, isEnter = false) {
    const table = document.getElementById(tableId);
    if (!table) return;
    
    const tbody = table.getElementsByTagName('tbody')[0];
    if (!tbody) return;
    
    const rows = Array.from(tbody.getElementsByTagName('tr'));
    const searchTerm = query.toLowerCase().trim();
    let visibleRowsCount = 0;
    let matchedRows = [];
    
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const cells = row.getElementsByTagName('td');
        
        if (cells.length === 1 && cells[0].getAttribute('colspan')) {
            row.style.display = searchTerm === "" ? "" : "none";
            continue;
        }

        let found = false;
        for (let j = 0; j < cells.length - 1; j++) {
            const cellText = cells[j].innerText.toLowerCase();
            if (cellText.includes(searchTerm)) {
                found = true;
                break;
            }
        }
        
        if (found || searchTerm === "") {
            row.style.display = "";
            row.classList.remove('row-hidden');
            visibleRowsCount++;
            if (found && searchTerm !== "") matchedRows.push(row.cloneNode(true));
        } else {
            row.style.display = "none";
            row.classList.add('row-hidden');
        }
    }
    
    if (visibleRowsCount === 0 && searchTerm !== "") {
        const emptyRows = rows.filter(row => {
            const cells = row.getElementsByTagName('td');
            return cells.length === 1 && cells[0].getAttribute('colspan');
        });
        emptyRows.forEach(row => row.style.display = "none");
    }
    
    updateTableTotals(tableId);

    // إذا تم الضغط على Enter وهناك نتائج، اعرضها في نافذة منبثقة
    if (isEnter && searchTerm !== "" && matchedRows.length > 0) {
        const modalTitle = `نتائج البحث عن: ${query}`;
        let modalContent = `
            <div class="search-results-modal">
                <p>تم العثور على ${matchedRows.length} نتيجة في ${tableId === 'workersTable' ? 'جدول العمال' : 'جدول العماير'}:</p>
                <div class="table-container">
                    <table class="dynamic-table">
                        <thead>${table.querySelector('thead').innerHTML}</thead>
                        <tbody></tbody>
                    </table>
                </div>
            </div>
        `;
        
        // إنشاء عنصر مؤقت لتحويل النص إلى DOM
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = modalContent;
        const modalTbody = tempDiv.querySelector('tbody');
        matchedRows.forEach(row => {
            // إزالة عمود الإجراءات من النتائج في النافذة المنبثقة ليكون العرض نظيفاً
            const cells = row.getElementsByTagName('td');
            if (cells.length > 0) {
                cells[cells.length - 1].innerHTML = '<span style="color:var(--primary-color)">نتيجة بحث</span>';
            }
            modalTbody.appendChild(row);
        });
        
        showModal(modalTitle, tempDiv.innerHTML);
    }
}

// --- ميزة البحث المتقدم (اختيارية) ---
function advancedFilterTable(tableId, query, columnIndexes = null) {
    const table = document.getElementById(tableId);
    if (!table) return;
    
    const tbody = table.getElementsByTagName('tbody')[0];
    if (!tbody) return;
    
    const rows = tbody.getElementsByTagName('tr');
    const searchTerm = query.toLowerCase().trim();
    
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const cells = row.getElementsByTagName('td');
        
        // تخطي صفوف "لا توجد بيانات"
        if (cells.length === 1 && cells[0].getAttribute('colspan')) {
            row.style.display = searchTerm === "" ? "" : "none";
            continue;
        }

        let found = false;
        
        if (columnIndexes && columnIndexes.length > 0) {
            // البحث في أعمدة محددة فقط
            for (let colIndex of columnIndexes) {
                if (colIndex < cells.length - 1) {
                    const cellText = cells[colIndex].innerText.toLowerCase();
                    if (cellText.includes(searchTerm)) {
                        found = true;
                        break;
                    }
                }
            }
        } else {
            // البحث في جميع الأعمدة ما عدا الأخيرة
            for (let j = 0; j < cells.length - 1; j++) {
                const cellText = cells[j].innerText.toLowerCase();
                if (cellText.includes(searchTerm)) {
                    found = true;
                    break;
                }
            }
        }
        
        row.style.display = (found || searchTerm === "") ? "" : "none";
        row.classList.toggle('row-hidden', !(found || searchTerm === ""));
    }
    
    updateTableTotals(tableId);
}

function updateTableTotals(tableId) {
    const table = document.getElementById(tableId);
    if (!table) return;
    
    const tbody = table.getElementsByTagName('tbody')[0];
    if (!tbody) return;

    const rows = Array.from(tbody.getElementsByTagName('tr'))
        .filter(row => row.style.display !== 'none');
        
    if (tableId === 'workersTable') {
        let totalRemaining = 0;
        rows.forEach(row => {
            const cells = row.getElementsByTagName('td');
            if (cells.length > 8) {
                const remainingCell = cells[8]; // عمود الباقي
                totalRemaining += parseNumber(remainingCell.innerText);
            }
        });
        
        const totalRemainingValue = document.getElementById('totalRemainingValue');
        if (totalRemainingValue) {
            totalRemainingValue.textContent = formatNumberWithSeparator(totalRemaining);
            // تحديث التفقيط إذا كانت الدالة موجودة
            const totalRemainingWords = document.getElementById('totalRemainingWords');
            if (totalRemainingWords) {
                if (typeof numberToArabicWords === 'function') {
                    totalRemainingWords.textContent = numberToArabicWords(totalRemaining);
                } else if (typeof numberToWords === 'function') {
                    totalRemainingWords.textContent = numberToWords(totalRemaining);
                }
            }
        }
    }
}

/* --- نظام التحكم في النوافذ الموحد (UI Windows) --- */
function openWindow(windowId) {
    playSound('click');
    // إغلاق جميع النوافذ المفتوحة في الصفحة الحالية
    document.querySelectorAll('.form-window').forEach(win => {
        win.style.display = 'none';
    });
    // فتح النافذة المطلوبة
    const targetWin = document.getElementById(windowId);
    if (targetWin) {
        targetWin.style.display = 'block';
        targetWin.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
}

function closeWindow(windowId) {
    playSound('click');
    const targetWin = document.getElementById(windowId);
    if (targetWin) targetWin.style.display = 'none';
}

// توافق مع المسميات القديمة لضمان عدم تعطل الكود
function openBuildingWindow(id) { openWindow(id); }
function closeBuildingWindow(id) { closeWindow(id); }

// تحديث دوال الإضافة لإغلاق النوافذ تلقائياً عند النجاح
const originalAddBuilding = addBuilding;
addBuilding = function() {
    originalAddBuilding();
    if (document.getElementById('buildingName').value === '') closeWindow('addBuildingWindow');
};

const originalAddDelivery = addDelivery;
addDelivery = function() {
    originalAddDelivery();
    if (document.getElementById('amountDelivered').value === '') closeWindow('addDeliveryWindow');
};

const originalAddWorker = addWorker;
addWorker = function() {
    originalAddWorker();
    if (document.getElementById('workerName').value === '') closeWindow('addWorkerWindow');
};

const originalAddWork = addWork;
addWork = function() {
    originalAddWork();
    if (document.getElementById('workerDelivered').value === '') closeWindow('addWorkWindow');
};

const originalAddPartnership = addPartnership;
addPartnership = function() {
    originalAddPartnership();
    if (document.getElementById('yahyaReceived').value === '') closeWindow('addPartnershipWindow');
};

// --- وظائف النسخ الاحتياطي التلقائي الجديدة ---

function updateAutoBackupSettings() {
    const enabled = document.getElementById('autoBackupToggle').checked;
    const interval = document.getElementById('autoBackupInterval').value;
    
    const settings = {
        enabled: enabled,
        interval: interval,
        lastBackup: localStorage.getItem('lastAutoBackupDate') || null
    };
    
    localStorage.setItem('autoBackupSettings', JSON.stringify(settings));
    document.getElementById('autoBackupOptions').style.display = enabled ? 'block' : 'none';
    
    if (enabled) {
        checkAndPerformAutoBackup();
    }
}

function loadAutoBackupSettings() {
    const settingsStr = localStorage.getItem('autoBackupSettings');
    if (settingsStr) {
        const settings = JSON.parse(settingsStr);
        document.getElementById('autoBackupToggle').checked = settings.enabled;
        document.getElementById('autoBackupInterval').value = settings.interval;
        document.getElementById('autoBackupOptions').style.display = settings.enabled ? 'block' : 'none';
        
        if (settings.lastBackup) {
            const lastDate = new Date(settings.lastBackup);
            document.getElementById('lastBackupInfo').innerText = `آخر نسخ تلقائي: ${lastDate.toLocaleDateString('ar-SA')} ${lastDate.toLocaleTimeString('ar-SA')}`;
        }
    }
}

function checkAndPerformAutoBackup() {
    const settingsStr = localStorage.getItem('autoBackupSettings');
    if (!settingsStr) return;
    
    const settings = JSON.parse(settingsStr);
    if (!settings.enabled) return;
    
    const now = new Date();
    let shouldBackup = false;
    
    if (!settings.lastBackup) {
        shouldBackup = true;
    } else {
        const lastBackupDate = new Date(settings.lastBackup);
        const diffTime = Math.abs(now - lastBackupDate);
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        
        if (settings.interval === 'weekly' && diffDays >= 7) {
            shouldBackup = true;
        } else if (settings.interval === 'monthly' && diffDays >= 30) {
            shouldBackup = true;
        }
    }
    
    if (shouldBackup) {
        console.log('Performing scheduled auto backup...');
        performScheduledBackup();
    }
}

async function performScheduledBackup() {
    try {
        const data = {
            buildings: loadFromStorage('buildings'),
            workers: loadFromStorage('workers'),
            partnerships: loadFromStorage('partnerships'),
            logo: localStorage.getItem('logo'),
            clearPassword: localStorage.getItem('clearPassword'),
            backupDate: new Date().toISOString()
        };
        
        const now = new Date();
        const dateStr = now.toLocaleDateString('ar-SA').replace(/\//g, '-');
        const fileName = `نسخة_تلقائية_دورية_${dateStr}.json`;
        const content = JSON.stringify(data, null, 2);
        
        // حفظ في التحميلات تلقائياً
        const blob = new Blob([content], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.style.display = 'none';
        a.href = url;
        a.download = fileName; 
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        
        // تحديث تاريخ آخر نسخ
        const settingsStr = localStorage.getItem('autoBackupSettings');
        const settings = JSON.parse(settingsStr);
        settings.lastBackup = now.toISOString();
        localStorage.setItem('autoBackupSettings', JSON.stringify(settings));
        localStorage.setItem('lastAutoBackupDate', now.toISOString());
        
        // تحديث الواجهة إذا كانت مفتوحة
        if (document.getElementById('lastBackupInfo')) {
            document.getElementById('lastBackupInfo').innerText = `آخر نسخ تلقائي: ${now.toLocaleDateString('ar-SA')} ${now.toLocaleTimeString('ar-SA')}`;
        }
        
        alert('تم إجراء النسخ الاحتياطي التلقائي الدوري بنجاح');
    } catch (error) {
        console.error('Scheduled Backup Error:', error);
    }
}

// إضافة استدعاء التحميل عند بدء التطبيق
const originalOnDOMContentLoaded = document.addEventListener;
// ملاحظة: بما أن هناك مستمع بالفعل في الملف، سنقوم بتعديله مباشرة في الخطوة التالية


// ===== دوال تخصيص الألوان والأيقونات =====

// الألوان الافتراضية
const DEFAULT_COLORS = {
    primary: '#0288d1',
    secondary: '#26c6da',
    success: '#43a047',
    danger: '#e53935'
};

// الأيقونات الافتراضية
const DEFAULT_ICONS = {
    dashboard: '📊',
    buildings: '🏗️',
    workers: '👷',
    partnership: '🤝',
    settings: '⚙️'
};

// تحميل الإعدادات المحفوظة عند تحميل الصفحة
function loadCustomizationSettings() {
    const savedColors = JSON.parse(localStorage.getItem('customColors') || '{}');
    const savedIcons = JSON.parse(localStorage.getItem('customIcons') || '{}');
    
    // تطبيق الألوان المحفوظة
    Object.keys(DEFAULT_COLORS).forEach(colorKey => {
        const colorValue = savedColors[colorKey] || DEFAULT_COLORS[colorKey];
        applyColorToCSS(colorKey, colorValue);
    });
    
    // تطبيق الأيقونات المحفوظة
    applyIconsToUI(savedIcons);
    
    // تحديث حقول الإدخال
    updateCustomizationInputs();
}

// تحديث حقول الإدخال بالقيم المحفوظة
function updateCustomizationInputs() {
    const savedColors = JSON.parse(localStorage.getItem('customColors') || '{}');
    const savedIcons = JSON.parse(localStorage.getItem('customIcons') || '{}');
    
    // تحديث حقول الألوان
    document.getElementById('primaryColorInput').value = savedColors.primary || DEFAULT_COLORS.primary;
    document.getElementById('primaryColorText').value = savedColors.primary || DEFAULT_COLORS.primary;
    
    document.getElementById('secondaryColorInput').value = savedColors.secondary || DEFAULT_COLORS.secondary;
    document.getElementById('secondaryColorText').value = savedColors.secondary || DEFAULT_COLORS.secondary;
    
    document.getElementById('successColorInput').value = savedColors.success || DEFAULT_COLORS.success;
    document.getElementById('successColorText').value = savedColors.success || DEFAULT_COLORS.success;
    
    document.getElementById('dangerColorInput').value = savedColors.danger || DEFAULT_COLORS.danger;
    document.getElementById('dangerColorText').value = savedColors.danger || DEFAULT_COLORS.danger;
    
    // تحديث حقول الأيقونات
    document.getElementById('dashboardIcon').value = savedIcons.dashboard || DEFAULT_ICONS.dashboard;
    document.getElementById('buildingsIcon').value = savedIcons.buildings || DEFAULT_ICONS.buildings;
    document.getElementById('workersIcon').value = savedIcons.workers || DEFAULT_ICONS.workers;
    document.getElementById('partnershipIcon').value = savedIcons.partnership || DEFAULT_ICONS.partnership;
    document.getElementById('settingsIcon').value = savedIcons.settings || DEFAULT_ICONS.settings;
    
    // تحديث المعاينة
    updateColorPreview();
}

// تطبيق اللون على CSS
function applyColorToCSS(colorName, colorValue) {
    const cssVarName = `--${colorName}-color`;
    document.documentElement.style.setProperty(cssVarName, colorValue);
}

// تطبيق الألوان من منتقي الألوان
function applyPrimaryColor() {
    const colorValue = document.getElementById('primaryColorText').value;
    if (/^#[0-9A-F]{6}$/i.test(colorValue)) {
        document.getElementById('primaryColorInput').value = colorValue;
        applyColorToCSS('primary', colorValue);
        saveCustomColors();
        updateColorPreview();
        playSound('click');
    } else {
        alert('يرجى إدخال لون صحيح بصيغة HEX (مثل: #0288d1)');
    }
}

function applySecondaryColor() {
    const colorValue = document.getElementById('secondaryColorText').value;
    if (/^#[0-9A-F]{6}$/i.test(colorValue)) {
        document.getElementById('secondaryColorInput').value = colorValue;
        applyColorToCSS('secondary', colorValue);
        saveCustomColors();
        updateColorPreview();
        playSound('click');
    } else {
        alert('يرجى إدخال لون صحيح بصيغة HEX (مثل: #0288d1)');
    }
}

function applySuccessColor() {
    const colorValue = document.getElementById('successColorText').value;
    if (/^#[0-9A-F]{6}$/i.test(colorValue)) {
        document.getElementById('successColorInput').value = colorValue;
        applyColorToCSS('success', colorValue);
        saveCustomColors();
        updateColorPreview();
        playSound('click');
    } else {
        alert('يرجى إدخال لون صحيح بصيغة HEX (مثل: #0288d1)');
    }
}

function applyDangerColor() {
    const colorValue = document.getElementById('dangerColorText').value;
    if (/^#[0-9A-F]{6}$/i.test(colorValue)) {
        document.getElementById('dangerColorInput').value = colorValue;
        applyColorToCSS('danger', colorValue);
        saveCustomColors();
        updateColorPreview();
        playSound('click');
    } else {
        alert('يرجى إدخال لون صحيح بصيغة HEX (مثل: #0288d1)');
    }
}

// تحديث اللون من حقل النص
function updateColorFromText(colorName) {
    const textInput = document.getElementById(`${colorName}ColorText`);
    const colorInput = document.getElementById(`${colorName}ColorInput`);
    const colorValue = textInput.value;
    
    // التحقق من صحة لون HEX
    if (/^#[0-9A-F]{6}$/i.test(colorValue)) {
        colorInput.value = colorValue;
        applyColorToCSS(colorName, colorValue);
        saveCustomColors();
        updateColorPreview();
    } else {
        alert('يرجى إدخال لون صحيح بصيغة HEX (مثل: #0288d1)');
    }
}

// حفظ الألوان المخصصة
function saveCustomColors() {
    const customColors = {
        primary: document.getElementById('primaryColorInput').value,
        secondary: document.getElementById('secondaryColorInput').value,
        success: document.getElementById('successColorInput').value,
        danger: document.getElementById('dangerColorInput').value
    };
    localStorage.setItem('customColors', JSON.stringify(customColors));
}

// إعادة تعيين الألوان الافتراضية
function resetColorsToDefault() {
    if (confirm('هل تريد إعادة تعيين جميع الألوان إلى الافتراضية؟')) {
        Object.keys(DEFAULT_COLORS).forEach(colorKey => {
            applyColorToCSS(colorKey, DEFAULT_COLORS[colorKey]);
        });
        localStorage.removeItem('customColors');
        updateCustomizationInputs();
        playSound('click');
        alert('تم إعادة تعيين الألوان بنجاح');
    }
}

// تطبيق الأيقونات المخصصة
function applyCustomIcons() {
    const customIcons = {
        dashboard: document.getElementById('dashboardIcon').value || DEFAULT_ICONS.dashboard,
        buildings: document.getElementById('buildingsIcon').value || DEFAULT_ICONS.buildings,
        workers: document.getElementById('workersIcon').value || DEFAULT_ICONS.workers,
        partnership: document.getElementById('partnershipIcon').value || DEFAULT_ICONS.partnership,
        settings: document.getElementById('settingsIcon').value || DEFAULT_ICONS.settings
    };
    
    localStorage.setItem('customIcons', JSON.stringify(customIcons));
    applyIconsToUI(customIcons);
    playSound('click');
    alert('تم تطبيق الأيقونات بنجاح');
}

// تطبيق الأيقونات على الواجهة
function applyIconsToUI(icons) {
    // تحديث أيقونات القائمة السفلية
    const navItems = document.querySelectorAll('.nav-item');
    if (navItems.length >= 5) {
        navItems[0].querySelector('.nav-icon').textContent = icons.dashboard || DEFAULT_ICONS.dashboard;
        navItems[1].querySelector('.nav-icon').textContent = icons.buildings || DEFAULT_ICONS.buildings;
        navItems[2].querySelector('.nav-icon').textContent = icons.workers || DEFAULT_ICONS.workers;
        navItems[3].querySelector('.nav-icon').textContent = icons.partnership || DEFAULT_ICONS.partnership;
        navItems[4].querySelector('.nav-icon').textContent = icons.settings || DEFAULT_ICONS.settings;
    }
    
    // تحديث عناوين الأقسام
    const dashboardTitle = document.querySelector('#dashboard h2');
    if (dashboardTitle) {
        dashboardTitle.textContent = (icons.dashboard || DEFAULT_ICONS.dashboard) + ' لوحة التحكم المتقدمة';
    }
    
    const buildingsTitle = document.querySelector('#buildings h2');
    if (buildingsTitle) {
        buildingsTitle.textContent = (icons.buildings || DEFAULT_ICONS.buildings) + ' إدارة العماير';
    }
    
    const workersTitle = document.querySelector('#workers h2');
    if (workersTitle) {
        workersTitle.textContent = (icons.workers || DEFAULT_ICONS.workers) + ' إدارة العمال';
    }
    
    const partnershipTitle = document.querySelector('#partnership h2');
    if (partnershipTitle) {
        partnershipTitle.textContent = (icons.partnership || DEFAULT_ICONS.partnership) + ' إدارة الشراكة';
    }
    
    const settingsTitle = document.querySelector('#settings h2');
    if (settingsTitle) {
        settingsTitle.textContent = (icons.settings || DEFAULT_ICONS.settings) + ' الإعدادات والنظام';
    }
}

// إعادة تعيين الأيقونات الافتراضية
function resetIconsToDefault() {
    if (confirm('هل تريد إعادة تعيين جميع الأيقونات إلى الافتراضية؟')) {
        localStorage.removeItem('customIcons');
        applyIconsToUI(DEFAULT_ICONS);
        updateCustomizationInputs();
        playSound('click');
        alert('تم إعادة تعيين الأيقونات بنجاح');
    }
}

// تحديث معاينة الألوان
function updateColorPreview() {
    const primaryColor = document.getElementById('primaryColorInput').value;
    const secondaryColor = document.getElementById('secondaryColorInput').value;
    const successColor = document.getElementById('successColorInput').value;
    const dangerColor = document.getElementById('dangerColorInput').value;
    
    document.getElementById('previewPrimaryColor').style.color = primaryColor;
    document.getElementById('previewSecondaryColor').style.color = secondaryColor;
    document.getElementById('previewSuccessColor').style.color = successColor;
    document.getElementById('previewDangerColor').style.color = dangerColor;
}

// استدعاء تحميل الإعدادات عند بدء التطبيق
document.addEventListener('DOMContentLoaded', function() {
    loadCustomizationSettings();
});

// إضافة مستمع للتغييرات الفورية على منتقي الألوان
document.addEventListener('DOMContentLoaded', function() {
    const colorInputs = ['primaryColorInput', 'secondaryColorInput', 'successColorInput', 'dangerColorInput'];
    colorInputs.forEach(inputId => {
        const input = document.getElementById(inputId);
        if (input) {
            input.addEventListener('input', function() {
                const colorName = inputId.replace('ColorInput', '');
                applyColorToCSS(colorName, this.value);
                updateColorPreview();
            });
        }
    });
});



// --- وظائف التعديل الجديدة ---

function openEditModal(type, id, subId) {
    playSound('click');
    let title = '';
    let content = '';
    let saveAction = '';

    if (type === 'delivery') {
        const buildings = loadFromStorage('buildings');
        const building = buildings.find(b => b.id === parseInt(id));
        // البحث بالمعرف الفريد أولاً, وإذا لم يوجد البحث بالفهرس
        let delivery = building.deliveries.find(d => d.id && d.id.toString() === subId.toString());
        if (!delivery) {
            // الفهرس كخطة احتياطية للبيانات القديمة
            const index = parseInt(subId);
            if (!isNaN(index) && index >= 0 && index < building.deliveries.length) {
                delivery = building.deliveries[index];
            }
        }
        
        if (!delivery) return alert('تعذر العثور على السجل');
        
        title = 'تعديل مسلمة - ' + building.name;
        content = `
            <div class="form-window" style="display: block; box-shadow: none; padding: 0;">
                <label>المبلغ المسلم:</label>
                <input id="editAmount" type="text" inputmode="numeric" value="${formatNumberWithSeparator(delivery.amount)}" onkeyup="formatNumberInput(this)">
                <label>المستلم:</label>
                <select id="editReceiver">
                    <option value="يحيى" ${delivery.receiver === 'يحيى' ? 'selected' : ''}>يحيى</option>
                    <option value="صدام" ${delivery.receiver === 'صدام' ? 'selected' : ''}>صدام</option>
                    <option value="أخرى" ${!['يحيى', 'صدام'].includes(delivery.receiver) ? 'selected' : ''}>أخرى</option>
                </select>
                <input id="editCustomReceiver" placeholder="اسم المستلم" style="display: ${!['يحيى', 'صدام'].includes(delivery.receiver) ? 'block' : 'none'};" value="${!['يحيى', 'صدام'].includes(delivery.receiver) ? delivery.receiver : ''}">
                <label>التاريخ:</label>
                <input id="editDate" type="date" value="${delivery.date.split('T')[0]}">
                <label>ملاحظة:</label>
                <input id="editNote" value="${delivery.note || ''}">
            </div>
        `;
        saveAction = `saveEdit('delivery', '${id}', '${subId}')`;
        
        // إضافة مستمع لتغيير المستلم
        setTimeout(() => {
            const select = document.getElementById('editReceiver');
            const custom = document.getElementById('editCustomReceiver');
            if (select && custom) {
                select.onchange = () => {
                    custom.style.display = select.value === 'أخرى' ? 'block' : 'none';
                };
            }
        }, 100);

    } else if (type === 'work') {
        const workers = loadFromStorage('workers');
        const worker = workers.find(w => w.id === parseInt(id));
        // البحث بالمعرف الفريد أولاً, وإذا لم يوجد البحث بالفهرس
        let work = worker.work.find(wk => wk.id && wk.id.toString() === subId.toString());
        if (!work) {
            // الفهرس كخطة احتياطية للبيانات القديمة
            const index = parseInt(subId);
            if (!isNaN(index) && index >= 0 && index < worker.work.length) {
                work = worker.work[index];
            }
        }
        
        if (!work) return alert('تعذر العثور على السجل');
        
        title = 'تعديل سجل عمل - ' + worker.name;
        const buildings = loadFromStorage('buildings');
        const buildingOptions = buildings.map(b => `<option value="${b.name}" ${work.building === b.name ? 'selected' : ''}>${b.name}</option>`).join('');
        
        content = `
            <div class="form-window" style="display: block; box-shadow: none; padding: 0;">
                <label>الفترة:</label>
                <input id="editPeriod" value="${work.period}">
                <label>المسلم:</label>
                <input id="editDelivered" type="text" inputmode="numeric" value="${formatNumberWithSeparator(work.delivered)}" onkeyup="formatNumberInput(this)">
                <label>إضافي:</label>
                <input id="editExtra" type="text" inputmode="numeric" value="${formatNumberWithSeparator(work.extra)}" onkeyup="formatNumberInput(this)">
                <label>مواصلات:</label>
                <input id="editTransport" type="text" inputmode="numeric" value="${formatNumberWithSeparator(work.transport)}" onkeyup="formatNumberInput(this)">
                <label>التاريخ:</label>
                <input id="editDate" type="date" value="${work.date.split('T')[0]}">
                <label>العمارة:</label>
                <select id="editBuilding">${buildingOptions}</select>
                <label>اليومية:</label>
                <input id="editDaily" type="text" inputmode="numeric" value="${formatNumberWithSeparator(work.daily)}" onkeyup="formatNumberInput(this)">
                <label>ملاحظة:</label>
                <input id="editNote" value="${work.note || ''}">
            </div>
        `;
        saveAction = `saveEdit('work', '${id}', '${subId}')`;

    } else if (type === 'worker') {
        const workers = loadFromStorage('workers');
        const worker = workers.find(w => w.name === id); // هنا id هو الاسم
        
        title = 'تعديل بيانات العامل - ' + worker.name;
        content = `
            <div class="form-window" style="display: block; box-shadow: none; padding: 0;">
                <label>اسم العامل:</label>
                <input id="editWorkerName" value="${worker.name}">
                <label>الأجر اليومي:</label>
                <input id="editWorkerWage" type="text" inputmode="numeric" value="${formatNumberWithSeparator(worker.wage)}" onkeyup="formatNumberInput(this)">
                <label>ملاحظة:</label>
                <input id="editWorkerNote" value="${worker.note || ''}">
            </div>
        `;
        saveAction = `saveEdit('worker', '${worker.name}', null)`;

    } else if (type === 'partnership') {
        const partnerships = loadFromStorage('partnerships');
        const d = partnerships.find(p => p.id === parseInt(id));
        
        title = 'تعديل عملية شراكة';
        const buildings = loadFromStorage('buildings');
        const buildingOptions = buildings.map(b => `<option value="${b.name}">${b.name}</option>`).join('');
        
        content = `
            <div class="form-window" style="display: block; box-shadow: none; padding: 0; max-height: 70vh; overflow-y: auto;">
                <label>التاريخ:</label>
                <input id="editDate" type="date" value="${d.date.split('T')[0]}">
                
                <h4 style="color: var(--primary-color); border-bottom: 1px solid #eee; padding-bottom: 5px;">يحيى:</h4>
                <label>المستلم:</label>
                <input id="editYahyaReceived" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.yahyaReceived)}" onkeyup="formatNumberInput(this)">
                <label>من عمارة:</label>
                <select id="editYahyaFrom"><option value="">اختر</option>${buildings.map(b => `<option value="${b.name}" ${d.yahyaReceivedFrom === b.name ? 'selected' : ''}>${b.name}</option>`).join('')}</select>
                <label>صرف عمال:</label>
                <input id="editYahyaPaid" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.yahyaPaidWorkers)}" onkeyup="formatNumberInput(this)">
                <label>مصاريف:</label>
                <input id="editYahyaExpenses" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.yahyaExpenses)}" onkeyup="formatNumberInput(this)">
                <label>حول لصدام:</label>
                <input id="editYahyaToSaddam" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.yahyaToSaddam)}" onkeyup="formatNumberInput(this)">

                <h4 style="color: var(--primary-color); border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 15px;">صدام:</h4>
                <label>المستلم:</label>
                <input id="editSaddamReceived" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.saddamReceived)}" onkeyup="formatNumberInput(this)">
                <label>من عمارة:</label>
                <select id="editSaddamFrom"><option value="">اختر</option>${buildings.map(b => `<option value="${b.name}" ${d.saddamReceivedFrom === b.name ? 'selected' : ''}>${b.name}</option>`).join('')}</select>
                <label>صرف عمال:</label>
                <input id="editSaddamPaid" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.saddamPaidWorkers)}" onkeyup="formatNumberInput(this)">
                <label>مصاريف:</label>
                <input id="editSaddamExpenses" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.saddamExpenses)}" onkeyup="formatNumberInput(this)">
                <label>حول ليحيى:</label>
                <input id="editSaddamToYahya" type="text" inputmode="numeric" value="${formatNumberWithSeparator(d.saddamToYahya)}" onkeyup="formatNumberInput(this)">
                
                <label>ملاحظة:</label>
                <input id="editNote" value="${d.note || ''}">
            </div>
        `;
        saveAction = `saveEdit('partnership', '${id}', null)`;
    }

    showModal(title, content, () => eval(saveAction), 'حفظ التعديلات');
}

function saveEdit(type, id, subId) {
    playSound('click');
    if (!confirm('هل أنت متأكد من حفظ التعديلات؟')) return;

    if (type === 'delivery') {
        let buildings = loadFromStorage('buildings');
        const building = buildings.find(b => b.id === parseInt(id));
        let deliveryIndex = building.deliveries.findIndex(d => d.id && d.id.toString() === subId.toString());
        if (deliveryIndex === -1) {
            const index = parseInt(subId);
            if (!isNaN(index) && index >= 0 && index < building.deliveries.length) {
                deliveryIndex = index;
            }
        }
        
        if (deliveryIndex === -1) return alert('تعذر العثور على السجل');
        
        const amount = parseNumber(document.getElementById('editAmount').value);
        const receiverSelect = document.getElementById('editReceiver').value;
        const receiver = receiverSelect === 'أخرى' ? document.getElementById('editCustomReceiver').value : receiverSelect;
        const date = document.getElementById('editDate').value;
        const note = document.getElementById('editNote').value;

        building.deliveries[deliveryIndex] = {
            ...building.deliveries[deliveryIndex],
            id: building.deliveries[deliveryIndex].id || Date.now(),
            amount,
            receiver,
            date: date + 'T12:00:00',
            note
        };
        saveToStorage('buildings', buildings);
        loadBuildingsTable();

    } else if (type === 'work') {
        let workers = loadFromStorage('workers');
        const worker = workers.find(w => w.id === parseInt(id));
        let workIndex = worker.work.findIndex(wk => wk.id && wk.id.toString() === subId.toString());
        if (workIndex === -1) {
            const index = parseInt(subId);
            if (!isNaN(index) && index >= 0 && index < worker.work.length) {
                workIndex = index;
            }
        }
        
        if (workIndex === -1) return alert('تعذر العثور على السجل');
        
        const period = document.getElementById('editPeriod').value;
        const delivered = parseNumber(document.getElementById('editDelivered').value);
        const extra = parseNumber(document.getElementById('editExtra').value);
        const transport = parseNumber(document.getElementById('editTransport').value);
        const date = document.getElementById('editDate').value;
        const building = document.getElementById('editBuilding').value;
        const daily = parseNumber(document.getElementById('editDaily').value);
        const note = document.getElementById('editNote').value;

        worker.work[workIndex] = {
            ...worker.work[workIndex],
            id: worker.work[workIndex].id || Date.now(),
            period,
            delivered,
            extra,
            transport,
            date: date + 'T12:00:00',
            building,
            daily,
            remaining: daily - delivered - extra - transport,
            note
        };
        saveToStorage('workers', workers);
        loadWorkersTable();

    } else if (type === 'worker') {
        let workers = loadFromStorage('workers');
        const worker = workers.find(w => w.name === id);
        
        const newName = document.getElementById('editWorkerName').value.trim();
        const wage = parseNumber(document.getElementById('editWorkerWage').value);
        const note = document.getElementById('editWorkerNote').value.trim();

        if (!newName || wage <= 0) return alert('بيانات غير صالحة');

        worker.name = newName;
        worker.wage = wage;
        worker.note = note;
        
        saveToStorage('workers', workers);
        loadWorkersTable();

    } else if (type === 'partnership') {
        let partnerships = loadFromStorage('partnerships');
        const index = partnerships.findIndex(p => p.id === parseInt(id));
        
        const date = document.getElementById('editDate').value;
        const yahyaReceived = parseNumber(document.getElementById('editYahyaReceived').value);
        const yahyaReceivedFrom = document.getElementById('editYahyaFrom').value;
        const yahyaPaidWorkers = parseNumber(document.getElementById('editYahyaPaid').value);
        const yahyaExpenses = parseNumber(document.getElementById('editYahyaExpenses').value);
        const yahyaToSaddam = parseNumber(document.getElementById('editYahyaToSaddam').value);
        
        const saddamReceived = parseNumber(document.getElementById('editSaddamReceived').value);
        const saddamReceivedFrom = document.getElementById('editSaddamFrom').value;
        const saddamPaidWorkers = parseNumber(document.getElementById('editSaddamPaid').value);
        const saddamExpenses = parseNumber(document.getElementById('editSaddamExpenses').value);
        const saddamToYahya = parseNumber(document.getElementById('editSaddamToYahya').value);
        
        const note = document.getElementById('editNote').value;

        partnerships[index] = {
            ...partnerships[index],
            date: date + 'T12:00:00',
            yahyaReceived, yahyaReceivedFrom, yahyaPaidWorkers, yahyaExpenses, yahyaToSaddam,
            saddamReceived, saddamReceivedFrom, saddamPaidWorkers, saddamExpenses, saddamToYahya,
            note
        };
        saveToStorage('partnerships', partnerships);
        loadPartnershipTable();
    }

    closeModal();
    playSound('success');
    alert('تم حفظ التعديلات بنجاح');
}

// --- نظام المزامنة السحابية (Cloud Sync) ---
// النسخة المحلية هي المرجع الأساسي (Offline-First)

function updateCloudSyncSettings() {
    const isEnabled = document.getElementById('cloudSyncToggle').checked;
    localStorage.setItem('cloudSyncEnabled', isEnabled);
    if (isEnabled) {
        checkCloudConnection();
    }
}

function checkCloudConnection() {
    const statusEl = document.getElementById('cloudSyncStatus');
    if (navigator.onLine) {
        statusEl.innerHTML = 'الحالة: <span style="color:var(--success-color)">متصل (جاهز للمزامنة)</span>';
    } else {
        statusEl.innerHTML = 'الحالة: <span style="color:var(--danger-color)">غير متصل (يعمل محلياً فقط)</span>';
    }
}

window.addEventListener('online', checkCloudConnection);
window.addEventListener('offline', checkCloudConnection);

async function uploadToCloud() {
    if (!navigator.onLine) {
        playSound('error');
        return alert('لا يوجد اتصال بالإنترنت حالياً');
    }
    
    playSound('click');
    const statusEl = document.getElementById('cloudSyncStatus');
    statusEl.innerHTML = 'جاري الرفع...';
    
    const allData = {
        buildings: loadFromStorage('buildings'),
        workers: loadFromStorage('workers'),
        partnerships: loadFromStorage('partnerships'),
        settings: {
            theme: localStorage.getItem('theme'),
            customColors: localStorage.getItem('customColors'),
            customIcons: localStorage.getItem('customIcons')
        },
        lastUpdated: new Date().toISOString()
    };

    try {
        // ملاحظة للمستخدم: هنا يتم ربط الـ API الخاص بـ Firebase أو أي خدمة أخرى
        // سنقوم بمحاكاة الرفع حالياً لتجهيز البنية التحتية
        console.log("Uploading data to cloud...", allData);
        
        // محاكاة تأخير الشبكة
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        localStorage.setItem('lastCloudSyncDate', new Date().toISOString());
        statusEl.innerHTML = 'آخر مزامنة (رفع): ' + new Date().toLocaleString('ar-SA');
        playSound('success');
        alert('تم رفع البيانات للسحابة بنجاح. النسخة المحلية تظل هي المرجع.');
    } catch (error) {
        console.error("Cloud upload error:", error);
        statusEl.innerHTML = '<span style="color:var(--danger-color)">فشل الرفع</span>';
        playSound('error');
    }
}

async function downloadFromCloud() {
    if (!navigator.onLine) {
        playSound('error');
        return alert('لا يوجد اتصال بالإنترنت حالياً');
    }

    if (!confirm('تحذير: تحميل البيانات من السحابة سيستبدل البيانات المحلية الحالية. هل تريد الاستمرار؟')) return;

    playSound('click');
    const statusEl = document.getElementById('cloudSyncStatus');
    statusEl.innerHTML = 'جاري التحميل...';

    try {
        // محاكاة تحميل البيانات
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        // في الواقع، هنا يتم جلب البيانات من الـ API
        // const response = await fetch('YOUR_CLOUD_API_ENDPOINT');
        // const cloudData = await response.json();
        
        statusEl.innerHTML = 'آخر مزامنة (تحميل): ' + new Date().toLocaleString('ar-SA');
        playSound('success');
        alert('تم تحميل البيانات من السحابة وتحديث النسخة المحلية.');
    } catch (error) {
        console.error("Cloud download error:", error);
        statusEl.innerHTML = '<span style="color:var(--danger-color)">فشل التحميل</span>';
        playSound('error');
    }
}

// تشغيل التحقق عند التحميل
document.addEventListener('DOMContentLoaded', () => {
    const isEnabled = localStorage.getItem('cloudSyncEnabled') === 'true';
    const toggle = document.getElementById('cloudSyncToggle');
    if (toggle) {
        toggle.checked = isEnabled;
        checkCloudConnection();
    }
    
    const lastSync = localStorage.getItem('lastCloudSyncDate');
    if (lastSync && document.getElementById('cloudSyncStatus')) {
        document.getElementById('cloudSyncStatus').innerHTML = 'آخر مزامنة: ' + new Date(lastSync).toLocaleString('ar-SA');
    }
});


// ============================================
// Firebase Integration (الربط الفعلي بـ Firebase)
// ============================================

let firebaseApp = null;
let firebaseDb = null;
let firebaseConfig = null;

// حفظ إعدادات Firebase
function saveFirebaseConfig() {
    const apiKey = document.getElementById('fbApiKey').value.trim();
    const projectId = document.getElementById('fbProjectId').value.trim();
    const dbUrl = document.getElementById('fbDbUrl').value.trim();
    
    if (!apiKey || !projectId || !dbUrl) {
        playSound('error');
        return alert('يرجى ملء جميع حقول Firebase');
    }
    
    firebaseConfig = { apiKey, projectId, dbUrl };
    localStorage.setItem('firebaseConfig', JSON.stringify(firebaseConfig));
    
    playSound('success');
    alert('تم حفظ إعدادات Firebase. سيتم استخدامها عند الرفع والتحميل.');
    
    // محاولة تهيئة Firebase
    initializeFirebase();
}

// تهيئة Firebase
async function initializeFirebase() {
    if (!firebaseConfig) {
        const saved = localStorage.getItem('firebaseConfig');
        if (!saved) return false;
        firebaseConfig = JSON.parse(saved);
    }
    
    try {
        // تحميل مكتبات Firebase من CDN
        if (!window.firebase) {
            // تحميل Firebase SDK
            const script1 = document.createElement('script');
            script1.src = 'https://www.gstatic.com/firebasejs/10.7.0/firebase-app.js';
            document.head.appendChild(script1);
            
            const script2 = document.createElement('script');
            script2.src = 'https://www.gstatic.com/firebasejs/10.7.0/firebase-database.js';
            script2.onload = () => {
                // بعد تحميل المكتبات
                const config = {
                    apiKey: firebaseConfig.apiKey,
                    projectId: firebaseConfig.projectId,
                    databaseURL: firebaseConfig.dbUrl
                };
                
                if (!firebaseApp) {
                    firebaseApp = firebase.initializeApp(config);
                    firebaseDb = firebase.database(firebaseApp);
                    console.log('Firebase initialized successfully');
                }
            };
            document.head.appendChild(script2);
        }
        
        return true;
    } catch (error) {
        console.error('Firebase initialization error:', error);
        return false;
    }
}

// رفع البيانات إلى Firebase
async function uploadToCloudFirebase() {
    if (!navigator.onLine) {
        playSound('error');
        return alert('لا يوجد اتصال بالإنترنت حالياً');
    }
    
    const config = localStorage.getItem('firebaseConfig');
    if (!config) {
        playSound('error');
        return alert('يرجى تكوين Firebase أولاً في الإعدادات');
    }
    
    playSound('click');
    const statusEl = document.getElementById('cloudSyncStatus');
    statusEl.innerHTML = 'جاري الرفع إلى Firebase...';
    
    try {
        const allData = {
            buildings: loadFromStorage('buildings'),
            workers: loadFromStorage('workers'),
            partnerships: loadFromStorage('partnerships'),
            settings: {
                theme: localStorage.getItem('theme'),
                customColors: localStorage.getItem('customColors'),
                customIcons: localStorage.getItem('customIcons')
            },
            lastUpdated: new Date().toISOString()
        };
        
        // إرسال البيانات إلى Firebase عبر API
        const firebaseConfigObj = JSON.parse(config);
        const response = await fetch(
            `${firebaseConfigObj.dbUrl}/accounting_data.json?auth=${firebaseConfigObj.apiKey}`,
            {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(allData)
            }
        );
        
        if (!response.ok) {
            throw new Error('Failed to upload to Firebase');
        }
        
        localStorage.setItem('lastCloudSyncDate', new Date().toISOString());
        statusEl.innerHTML = 'آخر مزامنة (رفع): ' + new Date().toLocaleString('ar-SA');
        playSound('success');
        alert('✅ تم رفع البيانات إلى Firebase بنجاح!');
        
    } catch (error) {
        console.error('Firebase upload error:', error);
        statusEl.innerHTML = '<span style="color:var(--danger-color)">❌ فشل الرفع</span>';
        playSound('error');
        alert('❌ حدث خطأ أثناء الرفع: ' + error.message);
    }
}

// تحميل البيانات من Firebase
async function downloadFromCloudFirebase() {
    if (!navigator.onLine) {
        playSound('error');
        return alert('لا يوجد اتصال بالإنترنت حالياً');
    }
    
    const config = localStorage.getItem('firebaseConfig');
    if (!config) {
        playSound('error');
        return alert('يرجى تكوين Firebase أولاً في الإعدادات');
    }

    if (!confirm('⚠️ تحذير: تحميل البيانات من Firebase سيستبدل البيانات المحلية الحالية. هل تريد الاستمرار؟')) {
        return;
    }

    playSound('click');
    const statusEl = document.getElementById('cloudSyncStatus');
    statusEl.innerHTML = 'جاري التحميل من Firebase...';

    try {
        const firebaseConfigObj = JSON.parse(config);
        const response = await fetch(
            `${firebaseConfigObj.dbUrl}/accounting_data.json?auth=${firebaseConfigObj.apiKey}`
        );
        
        if (!response.ok) {
            throw new Error('Failed to download from Firebase');
        }
        
        const cloudData = await response.json();
        
        if (!cloudData) {
            alert('لا توجد بيانات في Firebase');
            return;
        }
        
        // تحديث البيانات المحلية
        if (cloudData.buildings) saveToStorage('buildings', cloudData.buildings);
        if (cloudData.workers) saveToStorage('workers', cloudData.workers);
        if (cloudData.partnerships) saveToStorage('partnerships', cloudData.partnerships);
        
        // تحديث الإعدادات
        if (cloudData.settings) {
            if (cloudData.settings.theme) localStorage.setItem('theme', cloudData.settings.theme);
            if (cloudData.settings.customColors) localStorage.setItem('customColors', cloudData.settings.customColors);
            if (cloudData.settings.customIcons) localStorage.setItem('customIcons', cloudData.settings.customIcons);
        }
        
        // إعادة تحميل الجداول والواجهة
        loadBuildingsTable();
        loadWorkersTable();
        loadPartnershipTable();
        initCharts();
        
        statusEl.innerHTML = 'آخر مزامنة (تحميل): ' + new Date().toLocaleString('ar-SA');
        playSound('success');
        alert('✅ تم تحميل البيانات من Firebase وتحديث النسخة المحلية بنجاح!');
        
    } catch (error) {
        console.error('Firebase download error:', error);
        statusEl.innerHTML = '<span style="color:var(--danger-color)">❌ فشل التحميل</span>';
        playSound('error');
        alert('❌ حدث خطأ أثناء التحميل: ' + error.message);
    }
}

// تعديل دوال المزامنة الأصلية لاستخدام Firebase
const originalUploadToCloud = uploadToCloud;
const originalDownloadFromCloud = downloadFromCloud;

uploadToCloud = function() {
    const config = localStorage.getItem('firebaseConfig');
    if (config) {
        uploadToCloudFirebase();
    } else {
        originalUploadToCloud();
    }
};

downloadFromCloud = function() {
    const config = localStorage.getItem('firebaseConfig');
    if (config) {
        downloadFromCloudFirebase();
    } else {
        originalDownloadFromCloud();
    }
};

// تحميل إعدادات Firebase عند بدء التطبيق وتهيئة البيانات
document.addEventListener('DOMContentLoaded', () => {
    // تهيئة البيانات الأساسية فوراً لضمان عمل القوائم
    try {
        loadWorkersTable();
        loadBuildingsTable();
        loadBuildingLists();
        loadPartnershipTable();
    } catch(e) {
        console.error("Error initializing tables:", e);
    }

    const savedConfig = localStorage.getItem('firebaseConfig');
    if (savedConfig) {
        firebaseConfig = JSON.parse(savedConfig);
        const config = firebaseConfig;
        
        if (document.getElementById('fbApiKey')) {
            document.getElementById('fbApiKey').value = config.apiKey;
            document.getElementById('fbProjectId').value = config.projectId;
            document.getElementById('fbDbUrl').value = config.dbUrl;
        }
        
        // محاولة تهيئة Firebase
        initializeFirebase();
    }
});
